...
#define GET_DCU_FROM_LIST_DCU(index) \
                ((pbotDcu)(g_slist_nth(dculist,index))->data)
...
#define FREE_LIST_ZONE(free_func) { \
        if(zonelist) \
            bot_utils_free_slist(&zonelist, (GDestroyNotify)free_func); \
        if (zonemap) \
...
void osm_osm_gps_map_show_lights(pbotDcu dcu);
/** ------------------------------------------------------------------------------ **/
 /* ------------------------------------------------------------------------------ */

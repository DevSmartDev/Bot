...
static void      * smartstreetlight_event_thread_execute(void *thread_arg);
static void      * do_event_notify_popup(gpointer data);
static GtkWidget * smartstreetlight_event_create(gpointer data);
static void        spl_event_notify_window_focus (GtkWidget *widget,
                                                  GdkEvent  *event,
                                                  gpointer   user_data);
static void * spl_lcu_runtime_data_error_process(gpointer data);

typedef struct __SplNotification{
        ...
        if(g_mutex_trylock(&notify->lock))
        {
            ...
            if(selection)
            {
                if(gtk_tree_selection_get_selected(selection, &model, &iter))
                {
                    ...
                    //spl_utils_message_box(GTK_WINDOW(notify->dlg), GTK_MESSAGE_INFO, GTK_BUTTONS_OK, msg);
                    SPL_INFO_POPUP(GTK_WINDOW(notify->dlg), msg);
                    g_char_free(msg);
...
gboolean smartstreetlight_event_thread_init(void)
{
    ...
    if(!isInitThread)
    {
        ...
        notify = g_new0(SplNotification, 1);
        g_mutex_init(&notify->lock);
        notify->notification_nums = 0;
        notify->dlg = GTK_WIDGET(SPL_OBJECT("notification_dlg"));
        notify->view = GTK_TREE_VIEW(SPL_OBJECT("notification_view"));
        notify->notifilabel = GTK_LABEL(SPL_OBJECT("notification_lb"));
        notify->notifibtn = GTK_WIDGET(SPL_OBJECT("btn_show_notify"));
        ...
        g_signal_connect(SPL_OBJECT("notification_btn_quit"), "clicked",
...
void smartstreetlight_event_thread_finalize(void)
{
    if(is_started_thread==TRUE)
    {
        ...
        g_return_if_fail(spl_event_thread);
        if (g_thread_join(spl_event_thread) == 0)
        {
            DEBUG("\n smartstreetlight_event_thread_run: child thread exiting is OK.\n\n");
        }
...
void smartstreetlight_event_thread_run(void)
{
    ...
    #if 0
    ret = pthread_create(&spl_event_thread, NULL, smartstreetlight_event_thread_execute, NULL);
    if (ret)
    {
        GtkWidget *parent = stack_widget_active_top();
        SPL_ERROR_POPUP(parent, "Không thể tạo thread cập nhật dữ liệu runtime");
    }
    #else
    is_started_thread = TRUE;
    spl_event_thread = g_thread_new("event_thead", GThreadFunc(smartstreetlight_event_thread_execute), NULL);
    if (spl_event_thread==NULL)
    {
        is_started_thread = FALSE;
        GtkWidget *parent = stack_widget_active_top();
        SPL_ERROR_POPUP(parent, "Không thể tạo thread cập nhật dữ liệu runtime");
...
static void * spl_lcu_runtime_data_error_process(gpointer data)
{
    ...
    GtkWidget *parent = stack_widget_active_top();
    smartstreetlight_error_check_message_result(GTK_WIDGET(parent), msg, *((gint *)data));
    g_free(data);
    g_char_free(msg);
    return NULL;
}
...
static void *smartstreetlight_event_thread_execute(void *thread_arg)
{
    do
    {
        ...
        gint soap_err = spl_event_runtime_update_nofree(&spl_soap,&dcu_list, &dcu_list_size, &lcu_list, &lcu_list_size,
...
//        DEBUG("eventlog_list_size: %d", eventlog_list_size);
        if(soap_err==ns1__MESSAGE_USCORERESULT_USCOREEnum__NON_USCOREERR)
        {
            ...
            {
                spl_new_event_data *data = g_slice_new(spl_new_event_data);
            ...
            {
                spl_lcu_runtime_data *lcu_runtime_data;
                lcu_runtime_data = g_slice_new(spl_lcu_runtime_data);
                #if 1
...
//                DEBUG("=========> Recive LCU ID: %s, lightname: %s", lcu_list[i]->lcu_USCOREid, lcu_list[i]->lcu_USCORElightname);
                ...
                g_idle_add((GSourceFunc)spl_lcu_runtime_data_update, lcu_runtime_data);

            }

            for(gint i=0; i<dcu_list_size; i++)
            {
                spl_dcu_runtime_data *dcu_runtime_data;
                dcu_runtime_data = g_slice_new(spl_dcu_runtime_data);
                #if 1
                ...
                g_idle_add((GSourceFunc)spl_dcu_runtime_data_update, dcu_runtime_data);
            }
        }
        else
        {
            ...
            *result =soap_err;
            g_idle_add((GSourceFunc)spl_lcu_runtime_data_error_process, (gpointer)result);
        }
...
{
#if 0
    spl_new_event_data *event_data = (spl_new_event_data *)data;

    GtkWidget *window = smartstreetlight_event_create (event_data);
    ...
    g_slice_free(spl_new_event_data, event_data);
    return NULL;
#else
    ...
    spl_event_notify_add(0, content);
    g_char_free(content);
    //spl_utils_message_box(GTK_WINDOW(main_widgets->main_window), GTK_MESSAGE_WARNING,
...
//    gtk_window_set_position(GTK_WINDOW (window), GTK_WIN_POS_CENTER);
    ...
    g_signal_connect(GTK_WIDGET(window), "button-press-event", G_CALLBACK (spl_event_notify_window_focus), &window);

...
gboolean log_row_visible (GtkTreeModel *model, GtkTreeIter *iter, gpointer p)
{
    ...
    gint min, max;
    botLogPaging *page = (botLogPaging*)p;
...
static void on_btn_log_select_click (botDataObject *obj, gint btn)
{
    g_return_if_fail(obj && obj->paging);
    bot_event_log_selection_config(obj, TRUE, FALSE);
    switch(btn)
    {
    default:
        return;
...
{
    on_btn_log_select_click((botDataObject*) user_data, 1);
}

void on_event_log_btn_back_clicked(GtkButton *button, gpointer   user_data)
{
    on_btn_log_select_click((botDataObject*) user_data, 0);
}

gboolean on_image_selection_click(GtkImage *image, gpointer   user_data)
{
    botDataObject *obj = (botDataObject*)user_data;
    ...
    {
    case 0:
        bot_event_log_selection_config(obj, TRUE, TRUE);
        break;
...
static gboolean bot_user_log_callback_init(void)
{
    ...
    g_signal_connect(bot_OBJECT("btn_userlog_back"), "clicked",
                     G_CALLBACK(on_event_log_btn_back_clicked), &ObjData);
    g_signal_connect(bot_OBJECT("btn_userlog_next"), "clicked",
                     G_CALLBACK(on_event_log_btn_next_clicked), &ObjData);
    g_signal_connect(bot_OBJECT("userlog_btn_selection_click"), "clicked",
        ...
        if (bot_user_log_liststore_init())
        {
            if (bot_user_log_popup_menu_init())
            {
                if (bot_user_log_callback_init())
                {
                    ...
                    ObjData.update_information_func =   &bot_userlog_update_information;
                    ObjData.soap_delete_function =      &bot_delete_user_log_execute;
                    return TRUE;
                }
            }
        }
                                            ...
                                            gpointer user_data)
{
	...
	bot_event_log_selection_config(obj, FALSE, FALSE);
}
...
void bot_event_log_delete_marked_iter(botDataObject *obj)
{
    ...
    if(gtk_tree_model_get_iter_first(model, &sIter))
    {
        do
        {
            ...
            {
                botSoap bot_soap;
                gint soap_err = obj->soap_delete_function(&bot_soap, event_log_id);
                if(soap_err == ns1__MESSAGE_USCORERESULT_USCOREEnum__NON_USCOREERR)
                {
                    GtkTreeIter storeIter;
                    gtk_tree_model_filter_convert_iter_to_child_iter(obj->filter, &storeIter, &sIter);
                ...
                }else
                {
                    ...
                    GtkWidget *parent = stack_widget_active_top();
                    bot_error_check_message_result(GTK_WIDGET(parent), msgError->str, soap_err);
    ...
    bot_log_delete_row_marked(obj->store, obj->paging);
    gint maxPage = obj->paging->totalItem / MAX_PAGE_VIEW_NUM + 1;

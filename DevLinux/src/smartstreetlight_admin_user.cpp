...
static void spl_admin_user_manage_init_user_list(void);
static void spl_admin_user_manage_init_zone_list(void);

static void spl_admin_user_manage_init_specific_user(void);

static void spl_admin_user_manage_popup_menu_init();
static void spl_admin_user_manage_popup_menu_clicked(GtkWidget *popup_menu_item, gpointer userdata);
static void spl_admin_user_manage_popup_menu_clicked_add(void);
static void spl_admin_user_manage_popup_menu_clicked_edit(void);
static void spl_admin_user_manage_popup_menu_clicked_remove(void);
static void spl_admin_user_manage_popup_menu_show(GtkTreeView *treeview, GdkEventButton *event, gpointer userdata);
static gboolean on_admin_user_manage_treeview_clicked (GtkTreeView *treeview, GdkEventButton *event, gpointer userdata);

static void spl_admin_user_manage_zone_own_popup_menu_init();
static void spl_admin_user_manage_zone_own_popup_menu_clicked(void);
static void spl_admin_user_manage_zone_own_popup_menu_show(GtkTreeView *treeview, GdkEventButton *event, gpointer userdata);
...
static void spl_admin_user_manage_zone_own_change_run(gint user_id, gint count_all_zone);
static void spl_admin_user_manage_show_specific_user(GtkTreeModel *model, GtkTreeIter *iter);

static void spl_admin_user_manage_edit_init();

static void spl_admin_user_manage_edit_execute();

static void spl_admin_user_manage_add_execute();

static void spl_admin_user_manage_user_list_store_insert(ns1__USER_USCOREINFO_USCOREType *user_info);

static void spl_admin_user_manage_user_list_store_update(ns1__USER_USCOREINFO_USCOREType *user_info);
...
static void spl_admin_user_manage_user_info_fill(ns1__USER_USCOREINFO_USCOREType *user_info);

static void spl_admin_zone_list_update(ns1__ZONE_USCOREINFO_USCOREType *zone_info[], gint zone_size);

static gboolean is_current_user_belong_to(gint zone_id);

static gint spl_admin_zone_own_choose_fill_list_zone();
/** ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ **/
/** ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ **/
/** ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ **/
...
static void spl_admin_user_manage_init_specific_user(void)
{
    ...
    */
    ...
    admin_user_config_widget->email_allow_box
                                    = GTK_WIDGET(SPL_OBJECT("email_allow_box"));
    admin_user_config_widget->sms_allow_box
                                    = GTK_WIDGET(SPL_OBJECT("sms_allow_box"));
    ...
    admin_user_info_widget->admin_user_id_label
                                    = GTK_LABEL(SPL_OBJECT("admin_user_id_label"));
    admin_user_info_widget->admin_user_name_label
                                    = GTK_LABEL(SPL_OBJECT("admin_user_name_label"));
    admin_user_info_widget->admin_user_phone_label
                                    = GTK_LABEL(SPL_OBJECT("admin_user_phone_label"));
    admin_user_info_widget->admin_user_email_label
                                    = GTK_LABEL(SPL_OBJECT("admin_user_email_label"));
    admin_user_info_widget->admin_user_level_label
                                    = GTK_LABEL(SPL_OBJECT("admin_user_level_label"));
...
static void spl_admin_user_manage_init_user_list(void)
{
    ...
    */
    admin_user_list_widget->admin_user_treeview = GTK_WIDGET(SPL_OBJECT("admin_user_treeview"));
...
//    gtk_tree_view_append_column (GTK_TREE_VIEW(admin_user_list_widget->admin_user_treeview), column);
    ...
    gtk_tree_view_set_search_entry(GTK_TREE_VIEW(admin_user_list_widget->admin_user_treeview),
                                   GTK_ENTRY(SPL_OBJECT("admin_user_manage_search_entry")));
...
static void spl_admin_user_manage_init_zone_list(void)
{
    ...
    */
    admin_user_list_widget->admin_user_list_zone_treeview = GTK_WIDGET(SPL_OBJECT("admin_user_list_zone_treeview"));
    ...
    */
    ...
    g_signal_connect(GTK_TREE_VIEW(admin_user_list_widget->admin_user_list_zone_treeview),
        ...
        g_signal_connect (GTK_BUTTON(SPL_OBJECT("admin_user_zone_own_change_button")),
                     "clicked", G_CALLBACK (spl_admin_user_manage_zone_own_popup_menu_clicked), NULL);
}

gboolean spl_admin_user_manage_init(void)
{
    spl_admin_user_manage_init_user_list();
    spl_admin_user_manage_init_zone_list();
    spl_admin_user_manage_init_specific_user();
    spl_admin_user_manage_hide_specific_user_element();
    spl_admin_user_manage_edit_init();

    spl_admin_user_manage_popup_menu_init();
    spl_admin_user_manage_zone_own_popup_menu_init();

    gtk_paned_set_position(GTK_PANED(SPL_OBJECT("admin_user_manage_paned")), 350);

    gtk_paned_set_position(GTK_PANED(SPL_OBJECT("admin_user_specific_manage_paned")), 350);

    return TRUE;
}


gboolean spl_admin_user_manage_finalize(void)
{
...
gboolean spl_admin_user_manage_run(void)
{
    ns1__USER_USCOREINFO_USCOREType **user_list = NULL;
    gint user_list_size;
    SplSoap spl_soap;
    smartstreetlight_clear_list_store(GTK_TREE_MODEL(admin_user_list_widget->admin_user_liststore));
    gint soap_err = smartstreetlight_get_user_nofree_execute(&spl_soap,0,&user_list, &user_list_size);
    if (soap_err == ns1__MESSAGE_USCORERESULT_USCOREEnum__NON_USCOREERR)
    {
        if (user_list!=NULL)
        {
            ...
            {
                spl_admin_user_manage_user_list_store_insert(user_list[i]);
            }
        }
    } else {
...
static void spl_admin_user_manage_popup_menu_init()
{
    ...
    SPL_MENU_CSS(admin_user_manage_popup_menu->M_Popup);
    ...
    g_signal_connect(admin_user_manage_popup_menu->M_Add, "activate",
                 (GCallback) spl_admin_user_manage_popup_menu_clicked, GINT_TO_POINTER(ADMIN_USER_M_ADD));
    ...
    g_signal_connect(admin_user_manage_popup_menu->M_Edit, "activate",
                 (GCallback) spl_admin_user_manage_popup_menu_clicked, GINT_TO_POINTER(ADMIN_USER_M_EDIT));
    ...
    g_signal_connect(admin_user_manage_popup_menu->M_Remove, "activate",
                 (GCallback) spl_admin_user_manage_popup_menu_clicked, GINT_TO_POINTER(ADMIN_USER_M_REMOVE));
}
...
static void spl_admin_user_manage_popup_menu_clicked(GtkWidget *popup_menu_item, gpointer userdata)
{
    ...
    case ADMIN_USER_M_ADD:
        spl_admin_user_manage_popup_menu_clicked_add();
        break;
    case ADMIN_USER_M_EDIT:
        spl_admin_user_manage_popup_menu_clicked_edit();
        break;
    case ADMIN_USER_M_REMOVE:
        spl_admin_user_manage_popup_menu_clicked_remove();
        break;
    default:
        break;
    }
...
//    stack_widget_active_push(p->admin_user_manage_edit_dialog);
    do
    {
        SPL_DIALOG_SHOW(p->admin_user_manage_edit_dialog);
        ...
        switch(response_id)
        {
            case DIALOG_OK_RESPONSE:
            {
                ...
                if(is_ok)
                    spl_admin_user_manage_add_execute();
            }
            break;
    ...
    while(!is_ok);
    SPL_DIALOG_HIDE(p->admin_user_manage_edit_dialog);
...
static void spl_admin_user_manage_popup_menu_clicked_edit(void)
{
    ...
    if(spl_get_current_select_iter(treeview, &model, &iter))
    {
        ...
        {
            case DIALOG_OK_RESPONSE:
                spl_admin_user_manage_edit_execute();
                break;
            case DIALOG_CANCEL_RESPONSE:
                break;
        };
...
static void spl_admin_user_manage_popup_menu_clicked_remove(void)
{
    ...
    if(gtk_tree_selection_get_selected(selection, &model, &iter))
    {
        ...
        switch(SPL_QUESTION_POPUP(parent, buffer))
        {
            ...
            {
                SplSoap spl_soap;
                gint soap_err = smartstreetlight_remove_user_execute(&spl_soap,user_id);
                if(soap_err == ns1__MESSAGE_USCORERESULT_USCOREEnum__NON_USCOREERR)
                {
...
static void spl_admin_user_manage_show_user_specific(ns1__USER_USCOREINFO_USCOREType *user_info)
{
    GtkTreeIter iter;
...
static void spl_admin_user_manage_show_specific_user(GtkTreeModel *model, GtkTreeIter *iter)
{
    ...
    smartstreetlight_clear_tree_store(GTK_TREE_MODEL(admin_user_list_widget->admin_user_list_zone_treestore));
    SplSoap spl_soap;
    gint soap_err = smartstreetlight_get_user_nofree_execute(&spl_soap, user_id, &user_list, &user_list_size);
    if (soap_err == ns1__MESSAGE_USCORERESULT_USCOREEnum__NON_USCOREERR)
    {
        if (user_list!=NULL && user_list_size==1)
        {
            spl_admin_user_manage_show_user_specific(user_list[0]);
        }
    } else {
    ...
    if (event->type == GDK_2BUTTON_PRESS || event->type == GDK_BUTTON_PRESS)
    {
        ...
        if(event->type == GDK_2BUTTON_PRESS)
        {
            ...
            case GDK_BUTTON_PRIMARY:
                if(gtk_tree_selection_get_selected(selection, &model, &iter))
                {
                    spl_admin_user_manage_show_specific_user(model, &iter);
                }
                break;
            }
        }
        else if(event->type == GDK_BUTTON_PRESS)
        {
            ...
            case GDK_BUTTON_SECONDARY:
                spl_admin_user_manage_popup_menu_show(treeview, event, userdata);
                break;
            }
        }
    }
...
static void spl_admin_user_manage_edit_init()
{
    ...
    admin_user_manage_edit_widgets->admin_user_manage_edit_dialog = GTK_WIDGET(SPL_OBJECT("admin_user_manage_edit_dialog"));
    admin_user_manage_edit_widgets->admin_user_manage_edit_main_label = GTK_WIDGET(SPL_OBJECT("admin_user_manage_edit_main_label"));

    admin_user_manage_edit_widgets->admin_user_manage_edit_id_label = GTK_WIDGET(SPL_OBJECT("admin_user_manage_edit_id_label"));
    admin_user_manage_edit_widgets->admin_user_manage_edit_account_label = GTK_WIDGET(SPL_OBJECT("admin_user_manage_edit_account_label"));
    admin_user_manage_edit_widgets->admin_user_manage_edit_password_entry = GTK_WIDGET(SPL_OBJECT("admin_user_manage_edit_password_entry"));

    admin_user_manage_edit_widgets->admin_user_manage_edit_password_entry_2 = GTK_WIDGET(SPL_OBJECT("admin_user_manage_edit_password_entry_2"));
    admin_user_manage_edit_widgets->admin_user_manage_edit_retype_passwd_grid = GTK_WIDGET(SPL_OBJECT("admin_user_manage_edit_retype_passwd_grid"));

    admin_user_manage_edit_widgets->admin_user_manage_edit_phone_entry = GTK_WIDGET(SPL_OBJECT("admin_user_manage_edit_phone_entry"));
    admin_user_manage_edit_widgets->admin_user_manage_edit_email_entry = GTK_WIDGET(SPL_OBJECT("admin_user_manage_edit_email_entry"));

    admin_user_manage_edit_widgets->admin_user_manage_edit_email_notify_checkbutton = GTK_WIDGET(SPL_OBJECT("admin_user_manage_edit_email_notify_checkbutton"));
    admin_user_manage_edit_widgets->admin_user_manage_edit_sms_notify_checkbutton = GTK_WIDGET(SPL_OBJECT("admin_user_manage_edit_sms_notify_checkbutton"));

    admin_user_manage_edit_widgets->admin_user_manage_edit_level_comboboxtext = GTK_WIDGET(SPL_OBJECT("admin_user_manage_edit_level_comboboxtext"));

    admin_user_manage_edit_widgets->admin_user_manage_edit_manage_dcu_box = GTK_WIDGET(SPL_OBJECT("admin_user_manage_edit_manage_dcu_box"));
    admin_user_manage_edit_widgets->admin_user_manage_edit_manage_dcu_checkbutton = GTK_WIDGET(SPL_OBJECT("admin_user_manage_edit_manage_dcu_checkbutton"));

    admin_user_manage_edit_widgets->admin_user_manage_edit_create_program_checkbutton = GTK_WIDGET(SPL_OBJECT("admin_user_manage_edit_create_program_checkbutton"));

    admin_user_manage_edit_widgets->admin_user_manage_edit_manage_lcu_checkbutton = GTK_WIDGET(SPL_OBJECT("admin_user_manage_edit_manage_lcu_checkbutton"));
    admin_user_manage_edit_widgets->admin_user_manage_edit_setup_device_checkbutton = GTK_WIDGET(SPL_OBJECT("admin_user_manage_edit_setup_device_checkbutton"));
    admin_user_manage_edit_widgets->admin_user_manage_edit_control_lcu_checkbutton = GTK_WIDGET(SPL_OBJECT("admin_user_manage_edit_control_lcu_checkbutton"));
    admin_user_manage_edit_widgets->admin_user_manage_edit_email_checkbutton = GTK_WIDGET(SPL_OBJECT("admin_user_manage_edit_email_checkbutton"));
    admin_user_manage_edit_widgets->admin_user_manage_edit_sms_checkbutton = GTK_WIDGET(SPL_OBJECT("admin_user_manage_edit_sms_checkbutton"));

    admin_user_manage_edit_widgets->admin_user_manage_new_grid = GTK_WIDGET(SPL_OBJECT("admin_user_manage_new_grid"));
    admin_user_manage_edit_widgets->admin_user_manage_new_account_entry = GTK_WIDGET(SPL_OBJECT("admin_user_manage_new_account_entry"));

    admin_user_manage_edit_widgets->admin_user_manage_edit_grid = GTK_WIDGET(SPL_OBJECT("admin_user_manage_edit_grid"));

    admin_user_manage_edit_widgets->admin_user_manage_edit_error_label = GTK_WIDGET(SPL_OBJECT("admin_user_manage_edit_error_label"));
    ...
    gtk_widget_set_sensitive(GTK_WIDGET(SPL_OBJECT("admin_user_manage_dcu_checkbutton")), FALSE);
}
...
static void spl_admin_user_manage_add_execute()
{
    ...
    struct ADMIN_USER_MANAGE_EDIT_Widgets *p = admin_user_manage_edit_widgets;
    SplSoap spl_soap;
    spl_admin_user_manage_user_info_fill(&user_info);
    user_info.user_USCOREaccount = (gchar *)gtk_entry_get_text(GTK_ENTRY(p->admin_user_manage_new_account_entry));
    gint soap_err = smartstreetlight_add_user_execute(&spl_soap,user_info, &new_user_id);
    if(soap_err == ns1__MESSAGE_USCORERESULT_USCOREEnum__NON_USCOREERR)
    {
        user_info.user_USCOREid = new_user_id;
        spl_admin_user_manage_user_list_store_insert(&user_info);
    } else {
...
static void spl_admin_user_manage_edit_execute()
{
    ...
    gint soap_err = smartstreetlight_edit_user_execute(&spl_soap, user_info, FALSE);
    if(soap_err == ns1__MESSAGE_USCORERESULT_USCOREEnum__NON_USCOREERR)
    {
        spl_admin_user_manage_user_list_store_update(&user_info);
        if(g_strcmp0(gtk_label_get_text(admin_user_info_widget->admin_user_name_label), user_info.user_USCOREaccount)==0)
        {
            spl_admin_user_manage_show_user_specific(&user_info);
        }
    } else {
...
void spl_admin_user_manage_hide_specific_user_element()
{
    DEBUG_1();
...
static void spl_admin_user_manage_zone_own_popup_menu_init()
{
    ...
    SPL_MENU_CSS(admin_user_manage_zone_own_popup_menu->M_Popup);
    ...
    g_signal_connect(admin_user_manage_zone_own_popup_menu->M_Change, "activate",
                    (GCallback) spl_admin_user_manage_zone_own_popup_menu_clicked, NULL);
}
...
void spl_admin_zone_own_choose_init()
{
    if(admin_zone_own_choose_widget)
        return;

    admin_zone_own_choose_widget = g_slice_new(struct ADMIN_ZONE_OWN_Widgets);
    admin_zone_own_choose_widget->dialog = GTK_WIDGET(SPL_OBJECT("spl_admin_zone_own_choose_dialog"));
    admin_zone_own_choose_widget->treeview = GTK_WIDGET(SPL_OBJECT("spl_admin_zone_own_choose_treeview"));
...
static void spl_admin_user_manage_zone_own_change_run(gint user_id, gint count_all_zone)
{
    ...
    gint soap_err = smartstreetlight_edit_user_execute(&spl_soap, user_info, TRUE);
    if(soap_err == ns1__MESSAGE_USCORERESULT_USCOREEnum__NON_USCOREERR)
    {
        /* update for new zonelist */
        DEBUG_1();
        spl_admin_zone_list_update(zone_list, count_own_zone);
    }
    else
    {
...
static void spl_admin_user_manage_zone_own_popup_menu_clicked(void)
{
    ...
    {
        GtkWidget *parent = stack_widget_active_top();
        SPL_ERROR_POPUP(parent, "Xin chọn một tài khoản người dùng.");
        return;
    }
    ...
    gint count_all_zone = spl_admin_zone_own_choose_fill_list_zone();
    ...
    case DIALOG_OK_RESPONSE:
        {
            spl_admin_user_manage_zone_own_change_run(user_id, count_all_zone);
        }
        break;
    case DIALOG_CANCEL_RESPONSE:
        break;
                                                                ...
                                                                gpointer           userdata)
{
    ...
    if (event->type == GDK_BUTTON_PRESS)
    {
        ...
        if(event->type == GDK_BUTTON_PRESS)
        {
            ...
            {
            case GDK_BUTTON_SECONDARY:
                spl_admin_user_manage_zone_own_popup_menu_show(treeview, event, userdata);
                break;
            }
        }
    }
...
static void spl_admin_zone_list_update(ns1__ZONE_USCOREINFO_USCOREType *zone_info[], gint zone_size)
{
    ...
    smartstreetlight_clear_tree_store(GTK_TREE_MODEL(store));

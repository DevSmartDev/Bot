...
struct bot_ADMIN_ZONE_Widgets *bot_admin_zone_widgets;
struct ADMIN_ZONE_MANAGE_PopupMenu *admin_zone_manage_popupmenu;
struct bot_ADMIN_ZONE_EDIT_Widgets *admin_zone_edit_widgets;

//static void bot_admin_zone_treestore_fill_information();

static void bot_admin_zone_manage_popup_menu_init();

static void bot_admin_zone_edit_dialog_init(void);

static void bot_admin_zone_manage_popup_menu_clicked(GtkWidget *popup_menu_item, gpointer userdata);

static void bot_admin_zone_manage_popup_menu_clicked_add();

static void bot_admin_zone_manage_popup_menu_clicked_edit();

static void bot_admin_zone_manage_popup_menu_clicked_remove();

static void bot_admin_zone_manage_popup_menu_clicked_move();

static gint bot_admin_zone_move_fill_list(gint curr_zone_id);

static void bot_admin_zone_move_dcu_run(pbotZone old_zone, GtkTreeModel *main_model, GtkTreeIter *main_iter, pbotDcu dcu_info);

static void bot_admin_zone_list_treestore_insert(ns1__ZONE_USCOREINFO_USCOREType *zone_info);

static void bot_admin_zone_list_treestore_update(GtkTreeIter *iter, GtkTreeModel *model, ns1__ZONE_USCOREINFO_USCOREType *zone_info);
...
gboolean bot_admin_zone_manage_init(void)
{
    ...
    bot_admin_zone_widgets = g_slice_new(bot_ADMIN_ZONE_Widgets);

    bot_admin_zone_widgets->list_zone_treeview = GTK_WIDGET(bot_OBJECT("list_zone_treeview"));
    ...
    gtk_tree_view_append_column (GTK_TREE_VIEW(bot_admin_zone_widgets->list_zone_treeview), column);
...
*/
    ...
    bot_admin_zone_widgets->list_zone_treestore = gtk_tree_store_new(ADMIN_USER_ZONE_NUNBER_COLUMN,
                                                                        G_TYPE_STRING,
                                                                        G_TYPE_STRING,
                                                                        G_TYPE_INT,
                                                                        G_TYPE_INT,
    ...
    gtk_tree_view_set_model(GTK_TREE_VIEW(bot_admin_zone_widgets->list_zone_treeview),
                            GTK_TREE_MODEL(bot_admin_zone_widgets->list_zone_treestore));

    g_object_unref(G_OBJECT(bot_admin_zone_widgets->list_zone_treestore));


#endif // 0
    bot_admin_zone_edit_dialog_init();

    bot_admin_zone_manage_popup_menu_init();

    gtk_paned_set_position(GTK_PANED(bot_OBJECT("admin_zone_manage_paned")), 350);
...
static void bot_admin_zone_edit_dialog_init(void)
{
    admin_zone_edit_widgets = g_slice_new(bot_ADMIN_ZONE_EDIT_Widgets);

    admin_zone_edit_widgets->dialog = GTK_WIDGET(bot_OBJECT("admin_zone_edit_dialog"));
    admin_zone_edit_widgets->main_label = GTK_WIDGET(bot_OBJECT("admin_zone_edit_main_label"));
    admin_zone_edit_widgets->error_label = GTK_WIDGET(bot_OBJECT("admin_zone_edit_error_label"));
    admin_zone_edit_widgets->id_label = GTK_WIDGET(bot_OBJECT("admin_zone_edit_id"));
    admin_zone_edit_widgets->name_entry = GTK_WIDGET(bot_OBJECT("admin_zone_edit_name_entry"));
    admin_zone_edit_widgets->note_entry = GTK_WIDGET(bot_OBJECT("admin_zone_edit_note_entry"));
...
static void bot_admin_zone_manage_popup_menu_init()
{
    ...
    bot_MENU_CSS(admin_zone_manage_popupmenu->M_Popup);
    ...
    g_signal_connect(admin_zone_manage_popupmenu->M_Add, "activate",
                 (GCallback) bot_admin_zone_manage_popup_menu_clicked, GINT_TO_POINTER(ADMIN_ZONE_M_ADD));
    ...
    g_signal_connect(admin_zone_manage_popupmenu->M_Edit, "activate",
                 (GCallback) bot_admin_zone_manage_popup_menu_clicked, GINT_TO_POINTER(ADMIN_ZONE_M_EDIT));
    ...
    g_signal_connect(admin_zone_manage_popupmenu->M_Remove, "activate",
                 (GCallback) bot_admin_zone_manage_popup_menu_clicked, GINT_TO_POINTER(ADMIN_ZONE_M_REMOVE));
    ...
    g_signal_connect(admin_zone_manage_popupmenu->M_Move, "activate",
                 (GCallback) bot_admin_zone_manage_popup_menu_clicked, GINT_TO_POINTER(ADMIN_ZONE_M_MOVE));
}
...
void bot_admin_zone_manage_finalize(void)
{
    ...
    g_slice_free(bot_ADMIN_ZONE_EDIT_Widgets, admin_zone_edit_widgets);

    g_slice_free(ADMIN_ZONE_OWN_Widgets, admin_zone_own_choose_widget);
#if 0
    gtk_widget_destroy(GTK_WIDGET(bot_OBJECT("box93")));
    g_slice_free(bot_ADMIN_ZONE_Widgets, bot_admin_zone_widgets);
#endif // 0
}
...
//
//    gtk_notebook_set_current_page(GTK_NOTEBOOK(main_widgets->main_notebook), bot_NOTEBOOK_ZONE_MANAGE_PAGE);
//
...
gboolean bot_admin_zone_show_tab_on_notebook(void)
{
    ...
    if(osm_gps_map_change_parent("map_box", "admin_zone_map_box") &&
       bot_widget_change_parent(GTK_WIDGET(dcu_treeview), GTK_WIDGET(bot_OBJECT("scrolledwindow19")),
                                              GTK_WIDGET(bot_OBJECT("scrolledwindow23")))
      )
    {
...
//{
//    GtkTreeStore *store = GTK_TREE_STORE(bot_admin_zone_widgets->list_zone_treestore);
...
//                        {
////                            bot_admin_zone_manage_specific_show(zone_id);
...
//                bot_admin_zone_manage_popup_menu_show(treeview, event, userdata);
//                break;
//            }
//        }
//    }
...
static void bot_admin_zone_manage_popup_menu_clicked(GtkWidget *popup_menu_item, gpointer userdata)
{
    ...
    case ADMIN_ZONE_M_ADD:
        bot_admin_zone_manage_popup_menu_clicked_add();
        break;
    case ADMIN_ZONE_M_EDIT:
        bot_admin_zone_manage_popup_menu_clicked_edit();
        break;
    case ADMIN_ZONE_M_REMOVE:
        bot_admin_zone_manage_popup_menu_clicked_remove();
        break;
    case ADMIN_ZONE_M_MOVE:
        bot_admin_zone_manage_popup_menu_clicked_move();
        break;
    default:
        break;
    }
...
gboolean bot_admin_zone_manage_specific_show(pbotZone zone_data)
{
    ...
    osm_gps_map_show_lights_of_zone((pbotDcu)last_dcu->data);
...
static void bot_admin_zone_manage_popup_menu_clicked_add()
{
    ...
    struct bot_ADMIN_ZONE_EDIT_Widgets *p = admin_zone_edit_widgets;
    ...
    do
    {
        ...
        switch(response_id)
        {
            case DIALOG_OK_RESPONSE:
            {
                ...
                {
                    botSoap bot_soap;
                    gint soap_err = bot_add_zone_execute(&bot_soap,&zone_info, &new_zone_id);
                    if(soap_err == ns1__MESSAGE_USCORERESULT_USCOREEnum__NON_USCOREERR)
                    {
                        zone_info.zone_USCOREid = new_zone_id;
                        bot_admin_zone_list_treestore_insert(&zone_info);
                    }
                    else
                    {
        ...
        do
        {
            ...
            switch(response_id)
            {
                case DIALOG_OK_RESPONSE:
                {
                    ...
                    {
                        botSoap bot_soap;
                        gint soap_err = bot_edit_zone_execute(&bot_soap, &zone_info);
                        if(soap_err == ns1__MESSAGE_USCORERESULT_USCOREEnum__NON_USCOREERR)
                        {
                            bot_admin_zone_list_treestore_update(&iter, model, &zone_info);
                        }
                        else
                        {
...
static void bot_admin_zone_manage_popup_menu_clicked_remove()
{
    ...
    if(bot_get_current_select_iter(treeview, &model, &iter))
    {
        ...
        switch(bot_QUESTION_POPUP(parent, buffer))
        {
            ...
            {
                botSoap bot_soap;
                gint soap_err = bot_remove_zone_execute(&bot_soap, _zone->zonecode);
                if(soap_err == ns1__MESSAGE_USCORERESULT_USCOREEnum__NON_USCOREERR)
                {
...
static void bot_admin_zone_manage_popup_menu_clicked_move()
{
    ...
    if(bot_get_current_select_iter(treeview, &model, &iter))
    {
        ...
        count_all_zone = bot_admin_zone_move_fill_list(_zone->zonecode);
        ...
        case DIALOG_OK_RESPONSE:
            {
                bot_admin_zone_move_dcu_run(_zone, model, &iter, _dcu);
            }
            break;
        case DIALOG_CANCEL_RESPONSE:
            break;
...
static void bot_admin_zone_move_dcu_run(pbotZone old_zone, GtkTreeModel *main_model, GtkTreeIter *main_iter, pbotDcu dcu_info)
{
    ...
    if(gtk_tree_model_get_iter_first(model, &iter))
    {
        ...
        if(zone_id != -1)
        {
            ...
            botSoap bot_soap;
            gint soap_err = bot_edit_dcu_execute(&bot_soap, dcu_info);
            if(soap_err != ns1__MESSAGE_USCORERESULT_USCOREEnum__NON_USCOREERR)
            {
            ...
            else
            {
                ...
                {
                    GtkWidget *parent = stack_widget_active_top();
                    bot_ERROR_POPUP(parent, "Lỗi từ ứng dụng");
                }
            }
        }
    }
...
static void bot_admin_zone_list_treestore_insert(ns1__ZONE_USCOREINFO_USCOREType *zone_info)
{
    ...
    bot_zone_fill_infomation(zone_info->zone_USCOREid, zone_info->zone_USCOREname, zone_info->zone_USCOREnote);
}


static void bot_admin_zone_list_treestore_update(GtkTreeIter *iter, GtkTreeModel *model,
                                                 ns1__ZONE_USCOREINFO_USCOREType *zone_info)
{

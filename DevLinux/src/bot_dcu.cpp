...
/** ------------------------------------------------------------------------------ **/
//////////// DATA ///////////////
struct _botControlSchedule_Widgets *dcu_control_schedule_widgets;

////////// METHOD ///////////////
static void bot_dcu_control_init();

static void bot_dcu_control_finalize();
...
static void on_dcu_schedule_interleaved_changed_callback(GtkComboBox *widget,
        gpointer user_data);
static void bot_dcu_control_dialog_as_manual_show(pbotDcu dcu);
static void bot_dcu_control_dialog_as_schedule_show(pbotDcu dcu);
static void dcu_control_manual_show_hide(gboolean is_show);
static void dcu_control_schedule_show_hide(gboolean is_show);
static GSList * dcu_schedule_interleaved_prepare(pbotDcu dcu);
static pbotLcu dcu_schedule_interleaved_scheck_incense(GSList *lcu_list);
static void bot_dcu_control_schedule_get_lcu_list_with_state(GString **lights_is_on,
...
static void bot_dcu_control_window_accelerator_pressed(GtkWidget *widget, GdkEventKey *event, gpointer data);
/** **************************************************************************************** **/
...
void osm_gps_map_show_lights_of_zone(pbotDcu dcu)
{
    botZone * zone;
    pbotDcu dcuzone = NULL;
    GSList *itemDcu;
                ...
                for(itemDcu = zone->dculistzone; itemDcu != NULL; itemDcu = itemDcu->next)
                {
                    ...
                    if (dcuzone != NULL)
                    {
                        ...
                        LIST_LCU_FOR_EACH_ENTRY(dcuzone,item)
                        {
                            ...
                            bot_lcu_add_image(lcu, bot_get_light_icon(lcu, FALSE, dcu == dcuzone), &lat, &lon);
                        }
...
//                DEBUG("SHOW_ALL_DCU_IN_ZONE\n");
                ...
                LIST_LCU_FOR_EACH_ENTRY(dcuprev,item)
                {
                    ...
                    if (lcu->icon)
                    {
                        osm_gps_map_image_change_image(lcu->icon, bot_get_light_icon(lcu, FALSE, FALSE));
                    }
                }
                LIST_LCU_FOR_EACH_ENTRY(dcu,item)
                {
                    ...
                    if (lcu->icon)
                    {
                        osm_gps_map_image_change_image(lcu->icon, bot_get_light_icon(lcu, FALSE, TRUE));
                    }
                }
                dcuprev = dcu;
            }
...
void osm_gps_map_show_lights_of_dcu(pbotDcu dcu)
{
    gfloat lat = 0, lon = 0;
    ...
    LIST_LCU_FOR_EACH_ENTRY(dcu, item)
    {
        ...
        bot_lcu_add_image(lcu, bot_get_light_icon(lcu, FALSE,FALSE), &lat, &lon);
...
gboolean on_combo_box_spl_changed(GtkWidget *cbb, gpointer p)
{
    //DEBUG_1();
    gint index =  gtk_combo_box_get_active(GTK_COMBO_BOX(cbb));
    osm_osm_gps_map_show_lights(index, lastSelectedDcu);
//    gtk_widget_hide(index == DIbotAY_FOLLOW_DCU
//                    ? GTK_WIDGET(main_view_widgets->lcu_all_view)
//                    : GTK_WIDGET(main_view_widgets->lcu_view));
//
//    gtk_widget_show_all(index == DIbotAY_FOLLOW_DCU
//                        ? GTK_WIDGET(main_view_widgets->lcu_view)
//                        : GTK_WIDGET(main_view_widgets->lcu_all_view));
    return TRUE;
}

void osm_osm_gps_map_remove_dcu( pbotDcu dcu)
{
    if (dcu != NULL)
    {
        GSList *item;
...
void osm_osm_gps_map_show_lights(gint index, pbotDcu dcu)
{
    ...
    switch (index)
    {
    case DIbotAY_FOLLOW_DCU:
        DEBUG("DIbotAY_FOLLOW_DCU\n");
        ...
        break;
    case DIbotAY_FOLLOW_ZONE:
        DEBUG("DIbotAY_FOLLOW_ZONE\n");
        osm_gps_map_show_lights_of_zone(dcu);
        break;
    }
    if (SIZE_LIST_LCU(dcu) != 0)
    {
        pbotLcu lcu = (pbotLcu)lastSelectedDcu->priv->childlist->data;
        osm_gps_map_set_center(map, lcu->lat, lcu->lon);
    }
}
...
void bot_dcu_set_information(pbotDcu dcu)
{
    ...
    gtk_label_set_text(GTK_LABEL(gtk_builder_get_object (bot_builder, "lb_dcu_zone")),
                       dcu->priv->zonename);
    str = bot_get_status_mode(dcu->priv->status);
    //DEBUG("mode_name: %s\n",str);
    gtk_label_set_text(GTK_LABEL(gtk_builder_get_object (bot_builder, "lb_dcu_mode")),
                       str);
    g_free(str);
    str = bot_get_status_connect(dcu->priv->status);
    //DEBUG("connect_name: %s\n",str);
    gtk_label_set_text(GTK_LABEL(gtk_builder_get_object (bot_builder, "lb_dcu_connect")),
                       str);
    g_free(str);
    str = bot_get_status_name(dcu->priv->status);
    // DEBUG("status_name: %s\n",str);
    ...
    else
    {
                ...
                dcu->priv->programName == NULL)
        {
            ...
            gint soap_err = bot_get_program_list_nofree_execute(&bot_soap,dcu->priv->zonecode,dcu->priv->programId, &program, &program_size);
    ...
    gtk_label_set_text(GTK_LABEL(gtk_builder_get_object (bot_builder, "cbb_program_name")),
                       dcu->priv->programName);
        ...
        const gchar *zone_note)
{
    ...
    gtk_tree_store_set (main_view_widgets->dcu_store, &zone->zoneIter,
                        ...
                        -1);
    gint soap_err = bot_get_dcu_list_nofree_execute(&bot_soap,zone_id, 0, &dcu_list, &dcu_list_size);
    if (soap_err == ns1__MESSAGE_USCORERESULT_USCOREEnum__NON_USCOREERR)
    {
        if (dcu_list!=NULL)
        {
...
void bot_dcu_show_all()
{
    ...
    botSoap bot_soap;
    gint soap_err = bot_get_all_zone_nofree_execute(&bot_soap,&zone_list, &zone_list_size);
    if (soap_err ==ns1__MESSAGE_USCORERESULT_USCOREEnum__NON_USCOREERR)
    {
        if (zone_list != NULL)
        {
...
gboolean bot_dcu_add_new(void)
{
    ...
    bot_DIALOG_SHOW(main_widgets->add_dcu_dialog);
    gint response_id;
    while(response_id = gtk_dialog_run(GTK_DIALOG(main_widgets->add_dcu_dialog)))
    {
        ...
        case GTK_RESPONSE_ACCEPT:
        {
            ...
            else
            {
                ...
                //All things are ok
                botZone *zone1 = GET_ZONE_FROM_LIST_ZONE(gtk_combo_box_get_active(GTK_COMBO_BOX(add_dcu_combobox_zone)));
                ...
                gint soap_err = bot_add_dcu_execute(&bot_soap,dcu);
                if(soap_err == ns1__MESSAGE_USCORERESULT_USCOREEnum__NON_USCOREERR)
                {
...
___exit:
    bot_DIALOG_HIDE(main_widgets->add_dcu_dialog);
...
gboolean bot_dcu_edit(pbotDcu dcu)
{
    ...
    while(response_id = gtk_dialog_run(GTK_DIALOG(main_widgets->add_dcu_dialog)))
    {
        ...
        case GTK_RESPONSE_ACCEPT:
        {
            ...
            gint soap_err = bot_edit_dcu_execute(&bot_soap,dcuEdit);
            if(soap_err != ns1__MESSAGE_USCORERESULT_USCOREEnum__NON_USCOREERR)
            {
...
void bot_add_new_dcu_into_closest_list(pbotDcu dcu)
{
    while(g_slist_length(closestUsingDculist) >= (MAX_DCU_MAYBE -1))
    {
        ...
        //    FREE_LIST_LCU(lastDcu->dcu, bot_lcu_free);
        /*-------------- */
...
{
    bot_lcu_runtime_data *event_data = (bot_lcu_runtime_data *)data;
...
/**ADD ACTION*/
    if((event_data->lcu_status & ADD_STATUS) != 0)
    {
        ...
        //bot_error_notifice_run(main_widgets->main_window, buffer);
//                    bot_WARNING_POPUP(NULL, buffer);
        if(_dcu->priv->childlist != NULL)
        {
            pbotLcu new_lcu     = bot_lcu_init();
            ...
            if(lastSelectedDcu == _dcu)
            {
                ...
                bot_lcu_add_image(new_lcu, bot_get_light_icon(new_lcu, FALSE, FALSE),
            ...
            else
            {
                if(!IS_EMPTY_LIST_LCU(_dcu))
                {
                    ...
                    if(gtk_combo_box_get_active(GTK_COMBO_BOX(cbb_light_spl))==DIbotAY_FOLLOW_ZONE)
                    {
#if 0
                        store = main_view_widgets->lcu_all_store;
                        ...
                        gtk_tree_store_append(store, &iter, iter_parent);
                        gtk_tree_store_set(store, &iter, LIGHT_ID, new_lcu->id, LIGHT_POINTER, new_lcu, -1);
                        bot_lcu_add_image(new_lcu, bot_get_light_icon(new_lcu, FALSE, TRUE),
                                          &new_lcu->lat, &new_lcu->lon);
#endif
                    }
                }
...
//                            g_free(buffer);
            ...
            if(gtk_combo_box_get_active(GTK_COMBO_BOX(cbb_light_spl))!=DIbotAY_FOLLOW_DCU)
                bot_lcu_add_image(_lcu, bot_get_light_icon(_lcu, TRUE, TRUE), &_lcu->lat, &_lcu->lon);
            else
                bot_lcu_add_image(_lcu, bot_get_light_icon(_lcu, TRUE, FALSE), &_lcu->lat, &_lcu->lon);
        }
        else
        {
            if(gtk_combo_box_get_active(GTK_COMBO_BOX(cbb_light_spl))!=DIbotAY_FOLLOW_DCU)
            {
                ...
                if (_lcu->parent == lastSelectedDcu) {
                    bot_lcu_add_image(_lcu, bot_get_light_icon(_lcu, FALSE, TRUE), &_lcu->lat, &_lcu->lon);
                }else
                    bot_lcu_add_image(_lcu, bot_get_light_icon(_lcu, FALSE, FALSE), &_lcu->lat, &_lcu->lon);
...
__free:
    ...
    g_slice_free(bot_lcu_runtime_data, event_data);
...
{
    DEBUG_1();
    bot_dcu_runtime_data *event_data = (bot_dcu_runtime_data *)data;
    ...
    if((event_data->dcu_status & DELETE_STATUS) != 0)
    {
        if (is_found_dcu == TRUE)
        {
            ...
            if(gtk_tree_store_remove(main_view_widgets->dcu_store, &_dcu->priv->iter))
            {
                ...
                GtkTreeIter iter;
                selection = gtk_tree_view_get_selection(GTK_TREE_VIEW(main_view_widgets->dcu_view));
                bot_delete_all_lcu_on_dcu(_dcu);
...
//                g_free(buffer);
    ...
    g_slice_free(bot_dcu_runtime_data, event_data);
...
static void bot_dcu_control_finalize()
{
    ...
    {
        g_slist_free_full(dcu_control_schedule_widgets->dcu_schedule_lcu_sorted_list, GDestroyNotify(bot_lcu_free));
    ...
    g_slice_free(_botControlSchedule_Widgets, dcu_control_schedule_widgets);
}
...
static void bot_dcu_control_init()
{
    ...
    dcu_control_schedule_widgets = g_slice_new(_botControlSchedule_Widgets);
//    _botControlSchedule_Widgets *p = dcu_control_schedule_widgets;
    dcu_control_schedule_widgets->dcu_schedule_lcu_sorted_list = NULL;

    // common widgets //
    dcu_control_schedule_widgets->dcu_control_window               = GTK_WIDGET(bot_OBJECT("dcu_control_window"));
    dcu_control_schedule_widgets->dcu_control_core_frame_label     = GTK_WIDGET(bot_OBJECT("dcu_control_core_frame_label"));
    dcu_control_schedule_widgets->dcu_control_treeview             = GTK_WIDGET(bot_OBJECT("dcu_control_treeview"));
    dcu_control_schedule_widgets->dcu_control_select_button_box    = GTK_WIDGET(bot_OBJECT("dcu_control_select_button_box"));
    dcu_control_schedule_widgets->dcu_control_is_temp              = GTK_WIDGET(bot_OBJECT("dcu_control_is_temp"));

    // schedule setting widgets //
    dcu_control_schedule_widgets->dcu_schedule_program_box                 = GTK_WIDGET(bot_OBJECT("dcu_schedule_program_box"));
    dcu_control_schedule_widgets->dcu_schedule_program_comboboxtext        = GTK_WIDGET(bot_OBJECT("dcu_schedule_program_comboboxtext"));
    dcu_control_schedule_widgets->dcu_schedule_interleaved_comboboxtext    = GTK_WIDGET(bot_OBJECT("dcu_schedule_interleaved_comboboxtext"));
    dcu_control_schedule_widgets->dcu_schedule_summit_button               = GTK_WIDGET(bot_OBJECT("dcu_schedule_summit_button"));

    // control maunal widgets //
    dcu_control_schedule_widgets->dcu_manual_level_grid            = GTK_WIDGET(bot_OBJECT("dcu_manual_level_grid"));
    dcu_control_schedule_widgets->dcu_manual_summit_button         = GTK_WIDGET(bot_OBJECT("dcu_manual_summit_button"));
...
//    GClosure* closure = g_cclosure_new(bot_dcu_control_window_accelerator_pressed, 0, 0);
...
//    gtk_window_add_accel_group(GTK_WINDOW(dcu_control_schedule_widgets->dcu_control_window), accel_group);
    g_signal_connect(GTK_WIDGET(dcu_control_schedule_widgets->dcu_control_window),
                     "key_press_event", G_CALLBACK(bot_dcu_control_window_accelerator_pressed), NULL);
    //signal
//    g_signal_connect(GTK_WIDGET(dcu_control_schedule_widgets->dcu_control_window),
...
//    DEBUG("DIALOG_CONTROL_IS_SCHEDULE: %d", DIALOG_CONTROL_IS_SCHEDULE);
    ...
    case DIALOG_CONTROL_IS_ON_OFF:
        bot_dcu_control_dialog_as_manual_show(dcu);
        break;
    case DIALOG_CONTROL_IS_SCHEDULE:
        bot_dcu_control_dialog_as_schedule_show(dcu);
        break;
    }
    ...
    bot_DIALOG_SHOW(dcu_control_schedule_widgets->dcu_control_window);
...
static void bot_dcu_control_dialog_as_manual_show(pbotDcu dcu)
{
    ...
    gtk_label_set_text(GTK_LABEL(bot_OBJECT("dcu_control_lb_title")), (gchar *)"Điều khiển trạm");
...
static void bot_dcu_control_dialog_as_schedule_show(pbotDcu dcu)
{
    ...
    gtk_label_set_text(GTK_LABEL(bot_OBJECT("dcu_control_lb_title")), (gchar *)"Lập lịch hoạt động");
    gtk_label_set_text(GTK_LABEL(dcu_control_schedule_widgets->dcu_control_core_frame_label ),
    ...
    if(zone_id!=0)
    {
        ...
        bot_clear_list_store(GTK_TREE_MODEL(store));
        botSoap bot_soap;
        gint soap_err= bot_get_program_list_nofree_execute(&bot_soap,zone_id, 0,&program, &program_size);
        if (soap_err == ns1__MESSAGE_USCORERESULT_USCOREEnum__NON_USCOREERR)
        {
            if (program != NULL)
            {
    ...
    for(temp = list_lcu_sorted; temp != NULL; temp = temp->next)
    {
        if(temp && temp->data)
        {
            ...
            if(lcu)
            {
                ...
                if(lcu->mode == bot_LCU_ScheduleMode && (current_program != -1))
                    is_active = TRUE;
...
void bot_dcu_control_spl_light_on_map()
{
    ...
    if(gtk_tree_model_get_iter_first(model, &iter))
    {
        do
        {
            ...
            bot_lcu_add_image(lcu, light_image[ offset + (toggled ? ICON_LIGHT_ON : ICON_LIGHT_OFF )], &lat, &lon);
        }
        while(gtk_tree_model_iter_next(model, &iter));
    }
    /*if(lat)
...
static GSList * dcu_schedule_interleaved_prepare(pbotDcu dcu)
{
    ...
    pbotLcu root_position_lcu =  dcu_schedule_interleaved_scheck_incense(lcu_list);
...
static pbotLcu dcu_schedule_interleaved_scheck_incense(GSList *lcu_list)
{
    ...
    pbotLcu lcu_min_lat = NULL, lcu_max_lat = NULL, lcu_min_lon = NULL, lcu_max_lon = NULL;
...
static void bot_schedule_dcu_execute(gpointer args)
{
    ...
    soap_response->result_value = bot_set_program_for_dcu_execute(&soap_thread.bot_soap,
                                                                               schedule_dcu->dcu_id,
                                                                               bot_DCU_WirelessMode,
                                                                               schedule_dcu->program_id);
}
static void* bot_schedule_dcu_receive(gpointer data)
{
//    DEBUG_1();
    ...
    {
        GtkWidget *parent = stack_widget_active_top();
        bot_INFO_POPUP(parent, "Yêu cầu lập lịch cho trạm thành công.");
        ...
        if(LOCKUP_LIST_DCU(schedule_dcu->dcu_id, _dcu))
        {
            if(_dcu && _dcu->priv)
            {
                ...
                _dcu->priv->status = _dcu->priv->status &(~(bot_DCU_MANUAL_MODE));
                bot_dcu_set_information(_dcu);
            }
        }
        on_dcu_control_btn_clicked(NULL, GINT_TO_POINTER(DIALOG_CONTROL_BTN_QUIT));
    }
...
static void bot_dcu_schedule_thread(const char *dcu_id,gboolean is_fail_ctrl)
{
    gint responseId;
    ...
    if(is_fail_ctrl==TRUE)
    {
        ...
        GtkWidget *parent = stack_widget_active_top();
        switch(responseId = bot_QUESTION_YES_NO_POPUP(parent, warning_str))
        {
    ...
    if (is_fail_ctrl==FALSE || is_continue==TRUE)
    {
        ...
        if(gtk_combo_box_get_active_iter(program_combobox, &program_iter))
        {
            ...
            if(soap_thread.init_thread(schedule_dcu, bot_schedule_dcu_execute,
                                                bot_schedule_dcu_receive ) == FALSE)
            {
...
static void bot_control_lcu_list_execute(gpointer args)
{
    SoapResponse *soap_response = (SoapResponse *)args;
    struct CtrlAllLcuInDcuThread* ctrl_list_lcu = (struct CtrlAllLcuInDcuThread *)soap_response->data;
    soap_response->result_value = bot_control_lcu_nofree(&soap_thread.bot_soap,ctrl_list_lcu->dcu_id,
...
static void* bot_control_lcu_list_receive(gpointer data)
{
    SoapResponse *soap_response = (SoapResponse *)data;
    ...
    {
        bot_dcu_schedule_thread(ctrl_list_lcu->dcu_id,is_fail_ctrl);
    }
...
{
    GtkWidget * dcu_id_label = GTK_WIDGET(bot_OBJECT("dcu_control_lb_id"));
    GtkWidget * dcu_control_is_temp = GTK_WIDGET(bot_OBJECT("dcu_control_is_temp"));
    ...
    if(mode_schedule != -1 && gtk_combo_box_get_active_iter(program_combobox, &program_iter))
    {
        ...
        bot_dcu_control_schedule_get_lcu_list_with_state(&lights_is_on, &lights_is_off);
        ctrl_list_lcu->dcu_id = g_strdup(dcu_id);
        ctrl_list_lcu->is_temp = is_temp;
        ctrl_list_lcu->scale = -1;
        ctrl_list_lcu->mode = bot_LCU_ScheduleMode; // set cho vui, khong tac dung
        ...
        if(soap_thread.init_thread(ctrl_list_lcu, bot_control_lcu_list_execute,
                                            bot_control_lcu_list_receive ) == FALSE)
        {
...
void bot_dcu_control_execute()
{
    ...
    GtkWidget * dcu_id_label = GTK_WIDGET(bot_OBJECT("dcu_control_lb_id"));
    GtkWidget * lcu_brightness_adjustment = GTK_WIDGET(bot_OBJECT("lcu_brightness_adjustment"));
    GtkWidget * dcu_control_is_temp = GTK_WIDGET(bot_OBJECT("dcu_control_is_temp"));
...
//    DEBUG("light_level: %d", light_level);
    ...
    ctrl_list_lcu->mode = bot_LCU_ManualMode;
    ctrl_list_lcu->lights_is_on = lights_is_on;
    ctrl_list_lcu->lights_is_off = lights_is_off;
...
static void bot_dcu_control_schedule_get_lcu_list_with_state(GString **lights_is_on, GString **lights_is_off)
{
    GtkTreeIter iter;
    GtkTreeModel *model = GTK_TREE_MODEL(dcu_control_schedule_widgets->dcu_control_lcu_list);
    gchar *lcu_id;
...
gboolean bot_dcu_info_show_tab_on_notebook()
{
    GtkWidget * dcu_treeview = GTK_WIDGET(main_view_widgets->dcu_view);

    if(osm_gps_map_change_parent("admin_zone_map_box", "map_box"))
    {
        if(bot_widget_change_parent(dcu_treeview, GTK_WIDGET(bot_OBJECT("scrolledwindow23")),
                                              GTK_WIDGET(bot_OBJECT("scrolledwindow19"))))
        {
//            DEBUG("\n\n ************** SET CURRENT_PARENT IS ADMIN_ZONE\n\n");
            g_object_set_data(G_OBJECT(dcu_treeview), "current_parent", (gpointer)"dcu_info_container");
            osm_gps_map_image_remove_all(map);

            osm_osm_gps_map_show_lights(gtk_combo_box_get_active(GTK_COMBO_BOX(cbb_light_spl)),
                                                         lastSelectedDcu);
...
void bot_zone_clear_is_show()
{
    GSList *item;
    LIST_ZONE_FOR_EACH_ENTRY(item)
    {
        pbotZone _zone = (pbotZone)item->data;

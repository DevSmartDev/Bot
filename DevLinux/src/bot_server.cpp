...
struct bot_SERVER_Widgets *server_widgets = NULL;

struct bot_SERVER_LOG_POPUP_MENU *server_log_popup_menu = NULL;

static gboolean bot_server_log_treeview_init(void);
static gboolean bot_server_log_liststore_init(void);
static gboolean bot_server_log_popup_menu_init(void);
static gboolean bot_server_log_callback_init(void);
static void bot_server_log_fill_infomation(gint id, gchar *content, gchar *time, gboolean readed);
static void on_server_log_button_clicked (GtkButton *button, gpointer user_data);
static void bot_server_log_set_selection_all(gboolean is_select);
static void on_server_log_renderer_toggled (GtkCellRendererToggle *cell_renderer,
                                            gchar *path,
                                            gpointer user_data);
static void bot_server_log_popupmenu_clicked(GtkWidget *popup_menu_item, gpointer userdata);

static gboolean on_bot_server_log_treeview_clicked (GtkTreeView *treeview,
                                    GdkEventButton     *event,
                                    gpointer           userdata);
static void bot_server_log_selection_toggle();
static void bot_server_log_delete_one_item();
static void bot_server_log_delete_mark_item();
static void bot_server_log_delete_all_item();
...
static botLogPaging paging  = {1, 1, 0, FALSE};

static botDataObject ObjData = {NULL, NULL, &paging, NULL};

static void bot_serverlog_update_information()
{
    gchar *curStr = g_strdup_printf("%d-%d", paging.minIndex, paging.maxIndex);
    gtk_label_set_text(GTK_LABEL(bot_OBJECT("lb_serverlog_cur")),curStr);
    gchar *total = g_strdup_printf("%d", paging.totalItem);
    gtk_label_set_text(GTK_LABEL(bot_OBJECT("lb_serverlog_total")), (gchar*)total);
    ...
    gtk_widget_set_sensitive(GTK_WIDGET(bot_OBJECT("btn_serverlog_back")), min);
    gtk_widget_set_sensitive(GTK_WIDGET(bot_OBJECT("btn_serverlog_next")), max);

}

static gboolean bot_server_log_treeview_init(void)
{
    server_widgets->server_log_treeview = GTK_TREE_VIEW(bot_OBJECT("server_log_treeview"));
    ...
    gtk_tree_view_set_search_entry(GTK_TREE_VIEW(server_widgets->server_log_treeview),
                                   GTK_ENTRY(gtk_builder_get_object (bot_builder, "server_log_search_entry")));
...
static gboolean bot_server_log_popup_menu_init(void)
{
    server_log_popup_menu = g_new0(struct bot_SERVER_LOG_POPUP_MENU, 1);

    GtkWidget *separator_item;
    server_log_popup_menu->M_POPUP = gtk_menu_new();
    bot_MENU_CSS(server_log_popup_menu->M_POPUP);
    ...
    g_signal_connect(server_log_popup_menu->M_DELETE, "activate",
                 (GCallback) bot_server_log_popupmenu_clicked, GINT_TO_POINTER(bot_SERVER_LOG_POPUP_MENU_DELETE));
...
//                 (GCallback) bot_server_log_popupmenu_clicked, GINT_TO_POINTER(bot_SERVER_LOG_POPUP_MENU_SELECT));
//
...
//                 (GCallback) bot_server_log_popupmenu_clicked, GINT_TO_POINTER(bot_SERVER_LOG_POPUP_MENU_DISSELECT));
//
...
/*
    ...
    g_signal_connect(server_log_popup_menu->M_SELECT_ALL, "activate",
                 (GCallback) bot_server_log_popupmenu_clicked, GINT_TO_POINTER(bot_SERVER_LOG_POPUP_MENU_SELECT_ALL));
    ...
    g_signal_connect(server_log_popup_menu->M_DISSELECT_ALL, "activate",
                 (GCallback) bot_server_log_popupmenu_clicked, GINT_TO_POINTER(bot_SERVER_LOG_POPUP_MENU_DISSELECT_ALL));
    ...
    g_signal_connect(server_log_popup_menu->M_DELETE_MARK, "activate",
                 (GCallback) bot_server_log_popupmenu_clicked, GINT_TO_POINTER(bot_SERVER_LOG_POPUP_MENU_DELETE_MARK));
    ...
    g_signal_connect(server_log_popup_menu->M_DELETE_ALL, "activate",
                 (GCallback) bot_server_log_popupmenu_clicked, GINT_TO_POINTER(bot_SERVER_LOG_POPUP_MENU_DELETE_ALL));
*/
    return TRUE;
}
static gboolean bot_server_log_callback_init(void)
{
    g_signal_connect(GTK_TREE_VIEW(server_widgets->server_log_treeview),
                     "button-press-event",
                     G_CALLBACK(on_bot_server_log_treeview_clicked), NULL);
    g_signal_connect(bot_OBJECT("btn_serverlog_back"), "clicked", G_CALLBACK(on_event_log_btn_back_clicked), &ObjData);
    g_signal_connect(bot_OBJECT("btn_serverlog_next"), "clicked", G_CALLBACK(on_event_log_btn_next_clicked), &ObjData);
    g_signal_connect(bot_OBJECT("serverlog_btn_selection_click"), "clicked", G_CALLBACK(on_image_selection_click), &ObjData);
    return TRUE;
}
...
gboolean bot_server_log_init(void)
{
    ...
    server_widgets = g_slice_new(bot_SERVER_Widgets);

    server_widgets->bot_server_log_box = GTK_WIDGET(bot_OBJECT("bot_server_log_box"));

    if(bot_server_log_treeview_init())
    {
        if (bot_server_log_liststore_init())
        {
            if (bot_server_log_popup_menu_init())
            {
                if (bot_server_log_callback_init())
                {
                    ObjData.filter = server_widgets->server_log_filter;
                    ObjData.store = server_widgets->server_log_liststore;
                    ObjData.update_information_func = &bot_serverlog_update_information;
                    ObjData.soap_delete_function = &bot_delete_event_log_execute;
                    ret = TRUE;
                }
            }
        }
...
//    gtk_widget_set_visible(GTK_WIDGET(bot_OBJECT("grid27")), FALSE);
...
{
    if(server_widgets->bot_server_log_box)
        gtk_widget_destroy(GTK_WIDGET(server_widgets->bot_server_log_box));
    g_slice_free(bot_SERVER_Widgets, server_widgets);
}
...
void bot_server_log_show_all(void)
{
    ...
    bot_clear_list_store(GTK_TREE_MODEL(server_widgets->server_log_liststore));
    ...
    gint soap_err = bot_get_event_log_nofree_execute(&bot_soap,device_id,
    ...
    if(soap_err == ns1__MESSAGE_USCORERESULT_USCOREEnum__NON_USCOREERR)
    {
        if (eventlog_list != NULL)
        {
            ...
            for(gint i=0; i<eventlog_list_size; i++)
            {
                ...
                bot_server_log_fill_infomation(event->eventlog_USCOREid,
    ...
    if(paging.totalItem < paging.maxIndex)
        paging.maxIndex = paging.totalItem;
    bot_serverlog_update_information();
...
static void bot_server_log_popupmenu_clicked(GtkWidget *popup_menu_item, gpointer userdata)
{
    ...
    {
    case bot_SERVER_LOG_POPUP_MENU_DELETE:
        bot_event_log_delete_marked_iter(&ObjData);
//        bot_server_log_delete_one_item();
        break;
    };
}
                                    ...
                                    gpointer           userdata)
{
    ...
    {
        bot_server_log_popupmenu_show(event, NULL);
    }
    return FALSE;
}

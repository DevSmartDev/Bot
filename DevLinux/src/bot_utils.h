#ifndef BOT_UTILS_H_INCLUDED
#define BOT_UTILS_H_INCLUDED
...
typedef enum bot_LISTSTORE_FILL_Enum
{
    LISTSTORE_PROGRAM_ALL_INFO = 0,
    LISTSTORE_PROGRAM_LCU_CONTROL
}bot_LISTSTORE_FILL_Enum;
...
typedef struct _botChartObject
{
    gboolean    isSave;
    gchar       *startTime;
    gchar       *endTime;
...
typedef struct bot_Object
{
    ...
    gchar           *server_url;
    gchar           *server_port;
    botChartObject  *chart;
} bot_Object;

typedef struct _botObjectMove
{
    ...
    GtkEntry        *lat_entry;
    GtkEntry        *lon_entry;
}botObjectMove, *pbotObjectMove;


typedef struct _botDataObject
{
...
extern bot_Object *bot_object;

extern bot_Widgets	*main_widgets;
extern GtkBuilder               *bot_builder;
...
void bot_gtk_parent_set_sensitive(GtkWidget *widget);

gint bot_utils_message_box(GtkWindow *parent, GtkMessageType type, GtkButtonsType btn, const gchar* msg);

gboolean bot_widget_change_parent(GtkWidget *object, GtkWidget *old_parent, GtkWidget *new_parent);

gboolean bot_utils_mkdir(const gchar *path, gboolean create_parent_dirs);

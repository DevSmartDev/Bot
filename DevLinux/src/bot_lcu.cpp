...
pbotDcu lastSelectedDcu = NULL;
GtkComboBox *cbb_light_spl = NULL;
pbotLcu lastSelectedLcu = NULL;

struct bot_LCU_POPUP_MENU *lcu_info_popup_menu = NULL;

struct bot_LCU_Widgets *lcu_widgets = NULL;

static void bot_view_init();

static gboolean bot_lcu_tree_view_init(void);

static gboolean bot_lcu_list_store_init(void);

void bot_lcu_dcu_list_combobox_fill(gchar *dcu_name);

static gboolean bot_lcu_popup_menu_init(void);

static void bot_lcu_popup_menu_widget_init(void);
static void bot_lcu_popup_menu_callback_init(void);

static void bot_lcu_popup_menu_clicked(GtkWidget *popup_menu_item, gpointer userdata);
static gboolean bot_lcu_popup_menu_add_clicked(void);
static gboolean bot_lcu_popup_menu_control_clicked(void);
static void bot_lcu_popup_menu_remove_clicked(void);
static void bot_lcu_popup_menu_remove_all_clicked(void);

/** ================================================================================================== **/
/** ================================================================================================== **/


static void  bot_lcu_add_soap_execute(gpointer args);
static void* bot_lcu_add_soap_receive(gpointer data);
static gboolean bot_lcu_add_soap_receive_process(pbotLcu lcu);

static void  bot_lcu_remove_soap_execute(gpointer args);
static void* bot_lcu_remove_soap_receive(gpointer data);
static void bot_lcu_remove_soap_receive_process(pbotLcu lcu);

static void bot_light_chart_show_execute(pbotLcu lcu, gchar *time_from, gchar *time_to);
...
static gboolean copy_light(pbotLcu pA, pbotLcu pB)
{
...
pbotLcuPopup lcuPopupMenu = NULL;
/**
*Callback functions
*
*
...
static void popup_light_view_init()
{
    ...
    {
        lcuPopupMenu = g_new0(botLcuPopup, 1);
        ...
        g_signal_connect(lcuPopupMenu->menu_add, "activate",
                         (GCallback) bot_lcu_popup_menu_clicked, GINT_TO_POINTER(bot_LCU_M_ADD));
        g_signal_connect(lcuPopupMenu->menu_add_from_file, "activate",
                         (GCallback) bot_lcu_popup_menu_clicked, GINT_TO_POINTER(bot_LCU_M_ADD_FROM_FILE));
        g_signal_connect(lcuPopupMenu->menu_edit, "activate",
                         (GCallback) bot_lcu_popup_menu_clicked, GINT_TO_POINTER(bot_LCU_M_EDIT));
        g_signal_connect(lcuPopupMenu->menu_remove, "activate",
                         (GCallback) bot_lcu_popup_menu_clicked, GINT_TO_POINTER(bot_LCU_M_REMOVE));
        g_signal_connect(lcuPopupMenu->menu_remove_all, "activate",
                         (GCallback) bot_lcu_popup_menu_clicked, GINT_TO_POINTER(bot_LCU_M_REMOVE_ALL));
        g_signal_connect(lcuPopupMenu->menu_control, "activate",
                         (GCallback) bot_lcu_popup_menu_clicked, GINT_TO_POINTER(bot_LCU_M_CONTROL));
        g_signal_connect(lcuPopupMenu->menu_location, "activate",
                         (GCallback) bot_lcu_popup_menu_clicked, GINT_TO_POINTER(bot_LCU_M_LOCATION));
        g_signal_connect(lcuPopupMenu->menu_chart, "activate",
                         (GCallback) bot_lcu_popup_menu_clicked, GINT_TO_POINTER(bot_LCU_M_CHART));
    }
}
...
void light_update_information(pbotLcu lcu, gboolean setmapcenter)
{
    ...
    gchar *str;
    gtk_label_set_text(GTK_LABEL(gtk_builder_get_object (bot_builder, "lb_lcu_id")), lcu->id);
    gtk_label_set_text(GTK_LABEL(gtk_builder_get_object (bot_builder, "lb_lcd_note")), lcu->note ? lcu->note : "");
    gtk_label_set_text(GTK_LABEL(gtk_builder_get_object (bot_builder, "lb_lcd_gps")), lcu->gps ?  lcu->gps : "");
    gtk_label_set_text(GTK_LABEL(gtk_builder_get_object (bot_builder, "lb_lcd_lightname")),
                       lcu->lightname? lcu->lightname : "");
    str = g_strdup_printf("%.2f W", lcu->volt/100);
    gtk_label_set_text(GTK_LABEL(gtk_builder_get_object (bot_builder, "lb_lcd_vol")),
                       str);
    g_free(str);
    str = g_strdup_printf("%.0f", lcu->brightness/10);
    DEBUG(" lcu->brightness: %f; str: %s\n",  lcu->brightness, str);
    gtk_label_set_text(GTK_LABEL(gtk_builder_get_object (bot_builder, "lb_lcd_level")),
                       str);
    g_free(str);
    str = bot_lcu_get_status_name(lcu->status);
    gtk_label_set_text(GTK_LABEL(gtk_builder_get_object (bot_builder, "lb_lcd_status")),
                       str);
    g_free(str);
    str = bot_lcu_get_mode_name(lcu->mode);
    gtk_label_set_text(GTK_LABEL(gtk_builder_get_object (bot_builder, "lb_lcd_mode")),
                       str);
    g_free(str);
    str = g_strdup_printf("%d W", lcu->power);
    gtk_label_set_text(GTK_LABEL(gtk_builder_get_object (bot_builder, "lb_lcd_power")),
                       str);
    g_free(str);
    str = g_strdup_printf("%d %%", lcu->tolerance);
    gtk_label_set_text(GTK_LABEL(gtk_builder_get_object (bot_builder, "lb_lcd_tolerance")),
                       str);
    g_free(str);
    gtk_switch_set_active(GTK_SWITCH(gtk_builder_get_object (bot_builder, "switch_light")), (lcu->isOn == TRUE));
    gtk_widget_set_sensitive(GTK_WIDGET(gtk_builder_get_object (bot_builder, "switch_light")), (lcu->status == 0));
    if(lcu->icon)
    {
        const OsmGpsMapPoint *pt = osm_gps_map_image_get_point(lcu->icon);
        if(lcu != lastSelectedLcu)
        {
            osm_gps_map_image_change_image(lcu->icon, bot_get_light_icon(lcu, TRUE, FALSE));

            if(lastSelectedLcu)
            {
                osm_gps_map_image_change_image(lastSelectedLcu->icon,
                                               bot_get_light_icon(lastSelectedLcu, FALSE,
                                               gtk_combo_box_get_active(GTK_COMBO_BOX(cbb_light_spl)) != DIbotAY_FOLLOW_DCU
...
gchar* bot_lcu_get_mode_name(gint mode)
{
    ...
    switch (mode)
    {
        case bot_LCU_ScheduleMode:
        case bot_LCU_OffMode:
            msg = g_strdup ("Lập lịch");
            break;
        case bot_LCU_ManualMode:
...
static void bot_view_init()
{
    ...
    if(!isInitView)
    {
        ...
        main_view_widgets->lcu_view
            = GTK_TREE_VIEW(gtk_builder_get_object (bot_builder, "light_info_tree_view"));
        main_view_widgets->lcu_all_view
            = GTK_TREE_VIEW(gtk_builder_get_object (bot_builder, "light_hidden_info_tree_view"));
...
{
    GtkTreeView *view = GTK_TREE_VIEW(gtk_builder_get_object (bot_builder, "dcu_info_tree_view"));
    ...
    if(showAllLightEnable)
    {
        ...
        gtk_widget_set_sensitive(GTK_WIDGET(cbb_light_spl), TRUE);
    }
    else
    {
        gtk_widget_set_sensitive(GTK_WIDGET(cbb_light_spl), TRUE);
    }
    gtk_combo_box_set_active(GTK_COMBO_BOX(cbb_light_spl), DIbotAY_FOLLOW_DCU);
...
{
    pbotLcuLog lcu_log = g_new0(botLcuLog, 1);
    lcu_log->lcuId = NULL;
...
{
    bot_view_init();
    if(cbb_light_spl == NULL)
    {
        cbb_light_spl = GTK_COMBO_BOX(gtk_builder_get_object (bot_builder, "cbb_light_spl"));

        //signal
        DEBUG("combobox signal init\n");
        g_signal_connect(gtk_builder_get_object (bot_builder, "cbb_light_spl"), "changed",
                         G_CALLBACK (on_combo_box_spl_changed), (gpointer) NULL);
        gtk_combo_box_set_active(GTK_COMBO_BOX(cbb_light_spl), DIbotAY_FOLLOW_DCU);
    }

    g_signal_connect(GTK_WIDGET(bot_OBJECT("switch_light")), "button-press-event", G_CALLBACK(on_light_switch_light_button_press), NULL);
...
GdkPixbuf *bot_get_light_icon(pbotLcu lcu, gboolean selected, gboolean parentSelected)
{
...
static void bot_lcu_add_soap_execute(gpointer args)
{
    ...
    pbotLcu lcu = (pbotLcu)soap_response->data;
    soap_response->result_value =  bot_add_lcu_execute(&soap_thread.bot_soap, lcu);
}
...
static void* bot_lcu_add_soap_receive(gpointer data)
{
    ...
    if(soap_response->result_value == ns1__MESSAGE_USCORERESULT_USCOREEnum__NON_USCOREERR)
    {
        ...
        bot_lcu_add_soap_receive_process((pbotLcu)soap_response->data);
    }
    else
    {
        gchar *msg = g_strdup_printf("Lỗi thêm đèn '%s' vào trạm '%s'.", lcu->id, lcu->parent->priv->id);
        GtkWidget *parent = stack_widget_active_top();
        bot_error_check_message_result(GTK_WIDGET(parent), msg, soap_response->result_value);
        bot_lcu_free((pbotLcu)soap_response->data);
        g_free(msg);
    }
...
{
    gboolean ret = FALSE;
    GtkWidget *add_lcu_entry_id = GTK_WIDGET(gtk_builder_get_object (bot_builder, "add_lcu_entry_id"));
    GtkWidget *add_lcu_entry_gps = GTK_WIDGET(gtk_builder_get_object (bot_builder, "add_lcu_entry_gps"));
    GtkWidget *add_lcu_entry_note = GTK_WIDGET(gtk_builder_get_object (bot_builder, "add_lcu_entry_note"));
    GtkWidget *add_lcu_entry_lightname = GTK_WIDGET(gtk_builder_get_object (bot_builder, "add_lcu_entry_lightname"));
    GtkWidget *add_lcu_entry_power = GTK_WIDGET(gtk_builder_get_object (bot_builder, "add_lcu_entry_power"));
    GtkWidget *add_lcu_entry_tolerance = GTK_WIDGET(gtk_builder_get_object (bot_builder, "add_lcu_entry_tolerance"));
    GtkWidget *add_lcu_combobox_dcu = GTK_WIDGET(gtk_builder_get_object (bot_builder, "add_lcu_combobox_dcu"));
    GtkWidget *add_lcu_label_error = GTK_WIDGET(gtk_builder_get_object (bot_builder, "add_lcu_label_error"));
    GtkWidget *lb_name = GTK_WIDGET(gtk_builder_get_object (bot_builder, "lb_dialog_lcu_name"));
    GtkWidget *btn_add = GTK_WIDGET(gtk_builder_get_object (bot_builder, "lcu_add_ok"));
    ...
    bot_DIALOG_SHOW(main_widgets->add_lcu_dialog);
    gint response_id;
        ...
        case GTK_RESPONSE_ACCEPT:
            DEBUG("\n accept click\n");
            {
                ...
                else
                {
                    ...
                    //All things are ok
                    pbotDcu dcu     = GET_DCU_FROM_LIST_DCU(gtk_combo_box_get_active(GTK_COMBO_BOX(add_lcu_combobox_dcu)));
                    pbotLcu lcu     = bot_lcu_init();
                    botSoap bot_soap;
                    ...
                    if(soap_thread.init_thread(lcu, bot_lcu_add_soap_execute,
                                                    bot_lcu_add_soap_receive ) == FALSE)
                    {
                        gchar * str_error = g_strdup("Không thể tạo thread xử lý cho thao tác thêm mới LCU.");
                        DEBUG ("Internal error: %s", str_error);
                        GtkWidget *parent = stack_widget_active_top();
                        bot_error_check_message_result(GTK_WIDGET(parent), str_error, ns1__MESSAGE_USCORERESULT_USCOREEnum__INTERNAL_USCOREERR);
                        g_free(str_error);
                        ret = FALSE;
                    }
                    goto ___exit;
...
___exit:
    ...
    bot_DIALOG_HIDE(main_widgets->add_lcu_dialog);
...
{
    bot_object->chart->isSave = gtk_toggle_button_get_active(togglebutton);
}
...
static gboolean on_time_btn_click(GtkButton *button, gpointer userdata)
{
    ...
    {
        tm->tm_mon -= 1;
        GtkCalendar *calendar = GTK_CALENDAR(bot_OBJECT("calendar_select"));
        GtkWidget *dlg = GTK_WIDGET(bot_OBJECT("dialog_date_n_time"));
        if(calendar)
        {
            gtk_calendar_select_day(calendar, (guint)tm->tm_mday);
            gtk_calendar_select_month(calendar, (guint)tm->tm_mon, (guint)tm->tm_year);
            gtk_spin_button_set_value(GTK_SPIN_BUTTON(bot_OBJECT("spinbutton_hour")), (gdouble)tm->tm_hour);
            gtk_spin_button_set_value(GTK_SPIN_BUTTON(bot_OBJECT("spinbutton_minute")), (gdouble)tm->tm_min);
            gtk_spin_button_set_value(GTK_SPIN_BUTTON(bot_OBJECT("spinbutton_second")), (gdouble)tm->tm_sec);
        }
...
___LOOP1:
        if(gtk_dialog_run(GTK_DIALOG(dlg)))
        {
            ...
            tm->tm_hour = gtk_spin_button_get_value_as_int(GTK_SPIN_BUTTON(bot_OBJECT("spinbutton_hour")));
            tm->tm_min  = gtk_spin_button_get_value_as_int(GTK_SPIN_BUTTON(bot_OBJECT("spinbutton_minute")));
            tm->tm_sec  = gtk_spin_button_get_value_as_int(GTK_SPIN_BUTTON(bot_OBJECT("spinbutton_second")));
            time_t now = time(NULL);
            tm->tm_year -= EPOCH;
            if(now < mktime(tm))
            {
                GtkWidget *parent = stack_widget_active_top();
                bot_ERROR_POPUP(parent, "Thời gian không được lớn hơn thời gian hện tại!");
                goto ___LOOP1;
            }
        }
        bot_DIALOG_HIDE(dlg);
        ...
        {
            bot_object->chart->startTime = strTime;
        }
        else
        {
            bot_object->chart->endTime = strTime;
        }
...
gboolean bot_light_chart_show(pbotLcu lcu)
{
    static gboolean isInitChart = FALSE;
    g_return_if_fail(lcu);
    if(!isInitChart)
    {
        g_signal_connect(bot_OBJECT("autoload_time"), "toggled", G_CALLBACK(on_chart_remember_active), NULL);
        g_signal_connect(bot_OBJECT("slope_btn_start_time"), "clicked", G_CALLBACK(on_time_btn_click), GINT_TO_POINTER(0));
        g_signal_connect(bot_OBJECT("slope_btn_end_time"), "clicked", G_CALLBACK(on_time_btn_click), GINT_TO_POINTER(1));
        gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(bot_OBJECT("autoload_time")), bot_object->chart->isSave);
        isInitChart = TRUE;
    }
    if(bot_object->chart->isSave)
    {
        gtk_button_set_label(GTK_BUTTON(bot_OBJECT("slope_btn_start_time")), bot_object->chart->startTime);
        gtk_button_set_label(GTK_BUTTON(bot_OBJECT("slope_btn_end_time")), bot_object->chart->endTime);
    }
    else
    {
        time_t t = time(NULL);
        struct tm *tm = localtime(&t);
        gtk_button_set_label(GTK_BUTTON(bot_OBJECT("slope_btn_end_time")), g_strdup_printf(STR_DATETIME_FORMAT,
                                  tm->tm_year + EPOCH,
                                  tm->tm_mon + 1,
                                  tm->tm_mday,
                                  tm->tm_hour,
        ...
        t -= TIME_FORM_HERE;
        tm = localtime(&t);
        gtk_button_set_label(GTK_BUTTON(bot_OBJECT("slope_btn_start_time")), g_strdup_printf(STR_DATETIME_FORMAT,
                                  tm->tm_year + EPOCH,
                                  tm->tm_mon + 1,
                                  tm->tm_mday,
                                  tm->tm_hour,
    ...
    gint response_id;
    bot_DIALOG_SHOW(main_widgets->lcu_slope_datetime_dialog);
___LOOP:
    ...
    switch(response_id)
    {
        ...
        {
            gchar *startTime    = (gchar*) gtk_button_get_label(GTK_BUTTON(bot_OBJECT("slope_btn_start_time")));
            gchar *endTime      = (gchar*) gtk_button_get_label(GTK_BUTTON(bot_OBJECT("slope_btn_end_time")));
...

            {
                ...
                {
                    GtkWidget *parent = stack_widget_active_top();
                    bot_ERROR_POPUP(parent, "Thời gian kết thúc lớn hơn thời gian bắt đầu!");
                    goto ___LOOP;
                }
                bot_DIALOG_HIDE(main_widgets->lcu_slope_datetime_dialog);
                bot_light_chart_show_execute(lcu, startTime, endTime);
            }
        }
            break;
        default:
...
static void bot_light_chart_show_execute(pbotLcu lcu, gchar *time_from, gchar *time_to)
{
    ...
    TCHAR * cmd = g_strdup_printf("%s\\bin\\botchart -a \"%s\" -d %d -u \"%s\" -l \"%s\" -b \"%s\" -e \"%s\"",
...
#else
    gchar * cmd = g_strdup_printf("botchart -a \"%s\" -d %d -u \"%s\" -l \"%s\" -b \"%s\" -e \"%s\"",
                                                bot_username,
                                                bot_session_id,
                                                bot_server_url,
                                                lcu->id,
    ...
    while(response_id = gtk_dialog_run((GtkDialog *)main_widgets->add_lcu_dialog))
    {
        ...
        case GTK_RESPONSE_ACCEPT:
            DEBUG("accept click\n");
            {
                ...
                gint soap_err = bot_edit_lcu_execute(&bot_soap,lcuEdit);
...
{
    GtkWidget *manual_frame = GTK_WIDGET(bot_OBJECT("lcu_control_manual_mode_frame"));
    GtkWidget *schedule_frame = GTK_WIDGET(bot_OBJECT("lcu_control_schedule_mode_frame"));
...
static gboolean on_switch_button_press(GtkWidget *widget, GdkEvent *event, gpointer user_data)
{
    ...
    gtk_widget_set_sensitive(GTK_WIDGET(bot_OBJECT("lcu_control_scale_level")), !is_active);
    return FALSE;
}
...
static void bot_control_lcu_soap_execute(gpointer args)
{
    SoapResponse *soap_response = (SoapResponse *)args;
    struct CtrlLcuThread* ctrl_lcu = (struct CtrlLcuThread *)soap_response->data;
    GString *id = g_string_new(ctrl_lcu->lcu->id);
    soap_response->result_value = bot_control_lcu_nofree(&soap_thread.bot_soap,ctrl_lcu->lcu->parent->priv->id,
...
static void* bot_control_lcu_soap_receive(gpointer data)
{
    DEBUG_1();
    SoapResponse *soap_response = (SoapResponse *)data;
    struct CtrlLcuThread *ctrl_lcu = (struct CtrlLcuThread*)soap_response->data;
...
//            gfloat lat = 0, lon;
//            bot_lcu_add_image(ctrl_lcu->lcu, light_image[ctrl_lcu->lcu->isOn ? ICON_LIGHT_ON : ICON_LIGHT_OFF], &lat, &lon);
        }
        if (msgError->len != 0)
        {
            GtkWidget *parent = stack_widget_active_top();
            bot_ERROR_POPUP(parent, msgError->str);
        }
        else
        {
            GtkWidget *parent = stack_widget_active_top();
            bot_INFO_POPUP(parent, "Yêu cầu điều khiển các đèn của DCU thành công.");
        }
    }
    else
    {
    ...
    bot_free_soap_operation(&soap_thread.bot_soap);
    g_free((pbotLcu)soap_response->data);
    g_free(soap_response);
    return NULL;
}
gboolean bot_lcu_popup_menu_control(pbotLcu lcu)
{
    ...
    gtk_label_set_text(GTK_LABEL(bot_OBJECT("lcu_control_label_id")), lcu->id);
    gtk_label_set_text(GTK_LABEL(bot_OBJECT("lcu_control_label_lightname")), lcu->lightname);

    /* Will fix here later @alvinnguyen */
    DEBUG("lcu->mode %d", lcu->mode);
    gtk_combo_box_set_active(GTK_COMBO_BOX(bot_OBJECT("lcu_control_combobox_mode")), (lcu->mode-1));
    on_light_mode_changed(GTK_COMBO_BOX(bot_OBJECT("lcu_control_combobox_mode")), NULL);
    gtk_switch_set_active(GTK_SWITCH(bot_OBJECT("lcu_control_switch_on_off")), lcu->isOn);
    gtk_widget_set_sensitive(GTK_WIDGET(bot_OBJECT("lcu_control_scale_level")), lcu->isOn);
    gtk_adjustment_set_value(GTK_ADJUSTMENT(bot_OBJECT("lcu_brightness_adjustment")), (gdouble)lcu->brightness/10);
    if(!isSignalInit)
    {
        g_signal_connect(GTK_COMBO_BOX(bot_OBJECT("lcu_control_combobox_mode")), "changed",
                         G_CALLBACK(on_light_mode_changed), NULL);
        g_signal_connect(GTK_WIDGET(bot_OBJECT("lcu_control_switch_on_off")), "button-press-event",
                         G_CALLBACK(on_switch_button_press), NULL);
        isSignalInit = TRUE;
    }

    bot_DIALOG_SHOW(main_widgets->lcu_control_dialog);
    ...
    while(responseId = gtk_dialog_run(GTK_DIALOG(main_widgets->lcu_control_dialog)))
    {
        ...
        case GTK_RESPONSE_OK:
        {
            gboolean isOn = gtk_switch_get_active(GTK_SWITCH(bot_OBJECT("lcu_control_switch_on_off")));
            gint mode = gtk_combo_box_get_active(GTK_COMBO_BOX(bot_OBJECT("lcu_control_combobox_mode"))) + 1;
            gdouble  scale = gtk_adjustment_get_value(GTK_ADJUSTMENT(bot_OBJECT("lcu_brightness_adjustment")))*10;
            gboolean istemp = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(bot_OBJECT("lcu_control_istemp_checkbutton")));
            gboolean hasChange = FALSE;
            if(mode == lcu->mode)//truong hop ko thay doi mode
            {
                if(mode == bot_LCU_ManualMode)
                {
            ...
            {
                GtkWidget *parent = stack_widget_active_top();
                if(bot_QUESTION_POPUP(parent, "Chưa có thông số nào được thay đổi, bạn muốn thoát chứ?") == GTK_RESPONSE_CANCEL)
                    exit = FALSE;
            }
            else //cap nhat trang thai xuong device and sql
            {
//                if(mode == bot_LCU_ScheduleMode)
//                {
//                    isOn = lcu->isOn;
...
//                }
                ...
                if(soap_thread.init_thread(ctrl_lcu, bot_control_lcu_soap_execute,
                                                    bot_control_lcu_soap_receive ) == FALSE)
                {
    ...
    bot_DIALOG_HIDE(main_widgets->lcu_control_dialog);
    return TRUE;
}
...
static void  bot_lcu_remove_soap_execute(gpointer args)
{
    ...
    soap_response->result_value =  bot_remove_lcu_execute(&soap_thread.bot_soap, lcu->id, lcu->parent->priv->id);
}
...
static void* bot_lcu_remove_soap_receive(gpointer data)
{
    ...
    {
        // not free for lcu data //
        bot_lcu_remove_soap_receive_process((pbotLcu)soap_response->data);
        bot_lcu_free((pbotLcu)soap_response->data);
    }
    else
    {
...
void bot_light_remove_request(pbotLcu lcu)
{
    ...
    if(soap_thread.init_thread(lcu, bot_lcu_remove_soap_execute,
                                    bot_lcu_remove_soap_receive) == FALSE)
    {
...
void create_new_light_instances(pbotDcu dcu)
{
    ...
    gint lcu_list_size;
    botSoap bot_soap;
    gint soap_err = bot_get_lcu_list_nofree_execute(&bot_soap,dcu->priv->id, &lcu_list, &lcu_list_size);
    if (soap_err == ns1__MESSAGE_USCORERESULT_USCOREEnum__NON_USCOREERR)
    {
        if (lcu_list!= NULL)
        {
            ...
            for(gint i=0; i<lcu_list_size; i++)
            {
                ...
                pbotLcu lcuInstance    = g_new0(botLcu, 1);

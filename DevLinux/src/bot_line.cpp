...
struct bot_LINE_POPUP_MENU *line_info_popup_menu = NULL;

static gboolean bot_line_tree_view_init(void);

static gboolean bot_line_list_store_init(void);

static gboolean bot_line_callback_init(void);

static gboolean bot_lcu_popup_menu_init(void);

static void bot_line_popup_menu_widget_init(void);

static void bot_line_popup_menu_callback_init(void);

static void bot_line_popup_menu_clicked(GtkWidget *popup_menu_item, gpointer userdata);

static gboolean bot_line_popup_menu_add_clicked(void);

static void bot_line_popup_menu_remove_clicked(void);

static void bot_line_popup_menu_remove_all_clicked(void);
/** *********************************************************************************************** **/
...
static gboolean bot_line_popup_menu_add_clicked(void)
{
    ...
    GtkWidget *add_line_combobox_id = GTK_WIDGET(bot_OBJECT("add_line_combobox_id"));
    GtkWidget *add_line_entry_note = GTK_WIDGET(bot_OBJECT("add_line_entry_note"));
    GtkWidget *add_line_combobox_mode = GTK_WIDGET(bot_OBJECT("add_line_combobox_mode"));

    GtkWidget *add_line_label_error = GTK_WIDGET(bot_OBJECT("add_line_label_error"));

    gtk_widget_set_sensitive(GTK_WIDGET(main_widgets->main_window), FALSE);
    gint response_id;

    const gchar *line_dcuid = gtk_label_get_text(GTK_LABEL(bot_OBJECT("lb_dcu_id")));
        ...
        case DIALOG_OK_RESPONSE:
            ...
            else
            {
                ...
                else
                {
                    ...
                    gint soap_err = bot_add_line_execute(&bot_soap,&line_info);
                    if (soap_err == ns1__MESSAGE_USCORERESULT_USCOREEnum__NON_USCOREERR)
                    {
                        DEBUG_1();
                        bot_line_fill_infomation(line_info.line_USCOREid,
...
static void bot_line_popup_menu_remove_clicked(void)
{
    ...
    if (gtk_tree_selection_count_selected_rows(selection)  == 1)
    {
        if(gtk_tree_selection_get_selected(selection, &model, &iter))
        {
            ...
            gtk_label_set_text(GTK_LABEL(bot_OBJECT("delete_dialog_label")), "Xóa LINE với mã sau:");
            gtk_label_set_text(GTK_LABEL(bot_OBJECT("delete_element_id")), g_strdup_printf("%d", line_id));

            bot_DIALOG_SHOW(main_widgets->delete_element_dialog);
            gint response_id = gtk_dialog_run((GtkDialog *)main_widgets->delete_element_dialog);
            switch(response_id)
            {
                case DIALOG_OK_RESPONSE:
                {
                   gint soap_err = bot_remove_line_execute(&bot_soap,
                                                           line_id,
                                                           line_dcuid);
                   if(soap_err == ns1__MESSAGE_USCORERESULT_USCOREEnum__NON_USCOREERR)
                    {
            ...
            bot_DIALOG_HIDE(main_widgets->delete_element_dialog);
            g_free(line_dcuid);
        }
    }
}
...
static void bot_line_popup_menu_clicked(GtkWidget *popup_menu_item, gpointer userdata)
{
    bot_LINE_POPUP_MENU_Enum type = (bot_LINE_POPUP_MENU_Enum)GPOINTER_TO_INT(userdata);
    switch(type)
    {
    case bot_LINE_M_ADD:
        bot_line_popup_menu_add_clicked();
        break;
    case bot_LINE_M_REMOVE:
        bot_line_popup_menu_remove_clicked();
        break;
    case bot_LINE_M_REMOVE_ALL:
        bot_line_popup_menu_remove_all_clicked();
        break;
    default:
        break;
    }
...
static void bot_line_popup_menu_widget_init(void)
{
    ...
    bot_MENU_CSS(line_info_popup_menu->M_Popup);
...
static void bot_line_popup_menu_callback_init(void)
{
    g_signal_connect(line_info_popup_menu->M_Add, "activate",
                     (GCallback) bot_line_popup_menu_clicked, GINT_TO_POINTER(bot_LINE_M_ADD));

    g_signal_connect(line_info_popup_menu->M_Remove, "activate",
                     (GCallback) bot_line_popup_menu_clicked, GINT_TO_POINTER(bot_LINE_M_REMOVE));

    g_signal_connect(line_info_popup_menu->M_Remove_all, "activate",
                     (GCallback) bot_line_popup_menu_clicked, GINT_TO_POINTER(bot_LINE_M_REMOVE_ALL));
}

static gboolean bot_lcu_popup_menu_init(void)
{
    line_info_popup_menu = g_new0(struct bot_LINE_POPUP_MENU, 1);

    bot_line_popup_menu_widget_init();
    bot_line_popup_menu_callback_init();
...
static gboolean bot_line_tree_view_init(void)
{
    ...
    line_info_tree_view = GTK_TREE_VIEW(gtk_builder_get_object (bot_builder, "line_info_tree_view"));
...
{
    DEBUG("\n\n*************************** ENTER bot_lcu_info_init!! *******************\n\n");
    DEBUG_1();


    if(bot_line_tree_view_init())
    {
        if (bot_line_list_store_init())
        {
            if(bot_line_callback_init())
            {
                if(bot_lcu_popup_menu_init())
                {
                    return TRUE;
                }
            }
...
void bot_line_show_list(gchar *dcu_id)
{
    ...
    gint line_list_size;
    DEBUG_1();
    gint soap_err = bot_get_line_list_nofree_execute(&bot_soap,dcu_id, &line_list, &line_list_size);
    if (soap_err==ns1__MESSAGE_USCORERESULT_USCOREEnum__NON_USCOREERR)
    {


        bot_line_liststore_clean();
        if (line_list==NULL)
        {
            for(gint i = 0; i<line_list_size; i++)
            {
                ns1__LINE_USCOREINFO_USCOREType *line = line_list[i];
                bot_line_fill_infomation(line->line_USCOREid,
...
{
    bot_clear_list_store(GTK_TREE_MODEL(line_info_list_store));
}
...
static gboolean bot_line_callback_init(void)
{
    g_signal_connect(GTK_TREE_VIEW(line_info_tree_view),
                     "button-press-event",
                     G_CALLBACK(on_bot_line_specical_clicked), NULL);
    return TRUE;
}

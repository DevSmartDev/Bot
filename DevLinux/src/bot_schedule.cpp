...
GtkImage  *image_edit;
static botDaySchedule *dayTmp = NULL;
static botDayScheduleItem *itemTmp = NULL;
...
static int item_compare (const botDayScheduleItem *a, const botDayScheduleItem *b)
{
    return a->timeBegin - b->timeBegin;
}


static void bot_item_dialog_fill_tree(botDayScheduleItem *selectItem)
{
    ...
    {
        botDayScheduleItem  *it = (botDayScheduleItem*)list->data;
                         ...
                         gpointer         data)
{
    ...
    if(programChildListDay == NULL || g_slist_length(programChildListDay)== 0)
    {
        ...
        bot_WARNING_POPUP(parent, "Không có cấu hình chương trình theo ngày nào tồn tại");
        return;
    }
    ...
    {
        botDaySchedule *daySchedule = (botDaySchedule*)item->data;
        gtk_tree_store_append(store, &iter, NULL);
            ...
            if(gtk_tree_model_get_iter_first(GTK_TREE_MODEL(store), &iter))
            {
                do
                {
                    ...
                    if(check)
                    {
                        ...
                        {
                            //DEBUG("Id is 0\n");
                            gint soap_err = bot_add_day_schedule_execute(&bot_soap,programID, imonth, &Id);
                            ...
                            //DEBUG("get name: %s\n", name);
                            //change on database
                            //DEBUG("[[[bot_edit_day_schedule_execute(%d, %d, %d, %d)]]]\n",Id, scheduleId, programID, imonth);
                            gint soap_err = bot_edit_day_schedule_execute(&bot_soap, Id, scheduleId, programID, imonth);
                            if(soap_err != ns1__MESSAGE_USCORERESULT_USCOREEnum__NON_USCOREERR)
                            {
...
static void bot_program_schedule_fill_belong_information(gint progamId)
{
    ...
    botSoap bot_soap;
    gint soap_err = bot_get_list_schedule_nofree_execute(&bot_soap,0, progamId, &listschedule_list, &list_size);
    if (soap_err == ns1__MESSAGE_USCORERESULT_USCOREEnum__NON_USCOREERR)
    {
    ...
    /**get year schedule*/
    soap_err = bot_get_day_schedule_list_nofree_execute(&bot_soap,progamId, 0, &dayschedule_list, &list_size);
    if(soap_err ==ns1__MESSAGE_USCORERESULT_USCOREEnum__NON_USCOREERR)
    {
...
static void bot_schedule_item_modify(botDayScheduleItem *item)
{
    ...
    passSignal = TRUE;
    gtk_combo_box_set_active(GTK_COMBO_BOX(bot_OBJECT("cbb_startHour")), hStart);
    gtk_combo_box_set_active(GTK_COMBO_BOX(bot_OBJECT("cbb_endHour")), hEnd);
    gtk_combo_box_set_active(GTK_COMBO_BOX(bot_OBJECT("cbb_startMinute")), mStart);
    gtk_combo_box_set_active(GTK_COMBO_BOX(bot_OBJECT("cbb_endMinute")), mEnd);
    gtk_adjustment_set_value(GTK_ADJUSTMENT(bot_OBJECT("lcu_brightness_adjustment")), (gdouble)level);
    passSignal = FALSE;
...
{
    GSList *item = src;
    bot_utils_free_slist(dest, (GDestroyNotify)g_free);
    for( ; item != NULL; item = item->next)
    {
        botDayScheduleItem *itSrc = (botDayScheduleItem*)item->data;
        botDayScheduleItem *itDest = g_new0(botDayScheduleItem, 1);
        itDest->Id          = itSrc->Id;
...
static void bot_schedule_item_dialog_show(gint type, botDaySchedule *day, botDayScheduleItem *item)
{
    gint responseId;
    ...
    dayTmp = NULL;
    GtkListStore *store = schedule_item_widgets->schedule_item_content_liststore;
    gtk_widget_set_sensitive(GTK_WIDGET(bot_OBJECT("frame_config_schedule")), FALSE);
    switch(type)
    {
    case BTN_ITEM_ADD:
        gtk_label_set_text(GTK_LABEL(bot_OBJECT("lb_schedule_tittle")), "Tạo mới");
        gtk_entry_set_text(GTK_ENTRY(bot_OBJECT("schedule_item_name_entry")), "");
        nameEnable = TRUE;
        dayTmp = g_new0(botDaySchedule, 1);
        dayTmp->name = NULL;
        gtk_list_store_clear(store);
        break;
    case BTN_ITEM_EDIT:
        gtk_label_set_text(GTK_LABEL(bot_OBJECT("lb_schedule_tittle")), "Chỉnh sửa");
        if(!day || !day->name)
            return;
        //DEBUG("id: %s\n", day->name);
        dayTmp = day;
        gtk_list_store_clear(store);
        gtk_entry_set_text(GTK_ENTRY(bot_OBJECT("schedule_item_name_entry")), day->name);
        if(day->itemlist)
        {
            copy_item_child_list(day->itemlist, &day->itemlistTmp);
            bot_item_dialog_fill_tree(item);
            if(item)
                on_btn_item_clicked(NULL, GINT_TO_POINTER(BTN_ITEM_EDIT));
        }
        break;
    }
    gtk_widget_set_sensitive(GTK_WIDGET(bot_OBJECT("schedule_item_name_entry")), nameEnable);

    bot_DIALOG_SHOW(main_widgets->schedule_item_new_dialog);
        ...
        case GTK_RESPONSE_OK:
            switch (type)
            {
                case BTN_ITEM_ADD:
                {
                    ...
                    gchar *name = (gchar*)gtk_entry_get_text(GTK_ENTRY(bot_OBJECT("schedule_item_name_entry")));
                    if(!name  ||strlen(name) == 0)
                    {
                        GtkWidget *parent = stack_widget_active_top();
                        bot_WARNING_POPUP(parent, "Tên không được để trống");
                        goto ___loop;
                    }
                    GSList *list = programChildListDay;
                    for( ; list!= NULL; list = list->next)
                    {
                        botDaySchedule *cday = (botDaySchedule*)list->data;
                        if(cday)
                        {
                            if(g_strcmp0(cday->name, name) == 0)
                            {
                                GtkWidget *parent = stack_widget_active_top();
                                bot_WARNING_POPUP(parent, "Tên đã được sử dụng");
                                goto ___loop;
                            }
                        }
                    }
                ...
                {
                    botSoap bot_soap;
                    gint soap_err = bot_edit_list_schedule_execute(&bot_soap,dayTmp);
                    if(soap_err == ns1__MESSAGE_USCORERESULT_USCOREEnum__NON_USCOREERR)
                    {
                        DEBUG("update interface\n");
                        /**update interface*/
                        bot_program_schedule_fill_belong_information(dayTmp->programId);
                        goto ___exit;
                    }
                    else {
...
___exit:
    bot_DIALOG_HIDE(main_widgets->schedule_item_new_dialog);
...
static void on_schedule_item_btn_clicked(GtkButton *button,  gpointer userdata)
{
    ...
    case BTN_ITEM_EDIT:
        ...
        {
            day  = (botDaySchedule*)pObject;
            item = NULL;
        }
        else if(type == OBJECT_IS_ITEM_SCHEDULE_TYPE)
        {
            item = (botDayScheduleItem*)pObject;
            day = item->parent;
        }
    case BTN_ITEM_ADD:
        if(programID <= 0)
            break;
        bot_schedule_item_dialog_show(btn, day, item);
        break;
        ...
        if(hasSelected)
        {
            ...
            if(type == OBJECT_IS_DAY_SCHEDULE_TYPE)
            {
                if(pObject)
                {
                    ...
                    GtkWidget *parent = stack_widget_active_top();
                    if(bot_QUESTION_POPUP(parent, msg) == GTK_RESPONSE_OK)
                    {
                        gint soap_err = bot_remove_list_schedule_execute(&bot_soap,day->Id);
                        if(soap_err == ns1__MESSAGE_USCORERESULT_USCOREEnum__NON_USCOREERR)
                        {
                        ...
                        {
                            /*
                            bot_utils_message_box(GTK_WINDOW(main_widgets->main_window),
                                                  GTK_MESSAGE_WARNING,
                                                  GTK_BUTTONS_OK,
            ...
            else if(type == OBJECT_IS_ITEM_SCHEDULE_TYPE)
            {
                if(pObject)
                {
                    ...
                    if(bot_QUESTION_POPUP(parent, msg) == GTK_RESPONSE_OK)
                    {
                        gint soap_err = bot_remove_schedule_execute(&bot_soap,item->Id);
                        if(soap_err == ns1__MESSAGE_USCORERESULT_USCOREEnum__NON_USCOREERR)
                        {
                            gtk_tree_store_remove(program_widgets->program_item_schedule_treestore, &iter);
                            if(item->parent)
                            {
                                botDaySchedule *parent = item->parent;
...
static void bot_day_schedule_init_view()
{
    ...
    */
    ...
    //signal connect
    g_signal_connect(bot_OBJECT("schedule_btn_edit"), "clicked", G_CALLBACK(on_schedule_item_btn_clicked), GINT_TO_POINTER(BTN_ITEM_EDIT));
    g_signal_connect(bot_OBJECT("schedule_btn_add"), "clicked", G_CALLBACK(on_schedule_item_btn_clicked), GINT_TO_POINTER(BTN_ITEM_ADD));
    g_signal_connect(bot_OBJECT("schedule_btn_remove"), "clicked", G_CALLBACK(on_schedule_item_btn_clicked), GINT_TO_POINTER(BTN_ITEM_REMOVE));
}
...
void bot_program_fill_program_of_zone(botZone *zone, GtkTreeIter *parent)
{
    ...
    gint soap_err = bot_get_program_list_nofree_execute(&bot_soap,zone->zonecode,0, &program, &program_size);
    if (soap_err == ns1__MESSAGE_USCORERESULT_USCOREEnum__NON_USCOREERR)
    {
        if (program != NULL)
        {
...
static void on_combo_box_time_activated(GtkComboBoxText *cbbt, gpointer data)
{
    ...
    {
        botDayScheduleItem *it = (botDayScheduleItem*)item->data;
        if(it == itemTmp)
            continue;
...
static void btn_add_clicked()
{
    ...
    {
        GtkWidget *parent = stack_widget_active_top();
        bot_WARNING_POPUP(parent, "Không thể tạo hơn 10 lần bât tắt");
        return;
    }
    ...
    while(it)
    {
        botDayScheduleItem *item1 = (botDayScheduleItem*)it->data;
        ...
        if(next != NULL)
        {
            botDayScheduleItem *item2 = (botDayScheduleItem*)next->data;
        ...
        else
        {
            ...
            {
                GtkWidget *parent = stack_widget_active_top();
                bot_WARNING_POPUP(parent, "Không còn thời gian thích hợp");
                return;
            }
            else
            {
    ...
    passSignal = TRUE;
    gtk_combo_box_set_active(GTK_COMBO_BOX(bot_OBJECT("cbb_startHour")), hS);
    gtk_combo_box_set_active(GTK_COMBO_BOX(bot_OBJECT("cbb_endHour")), hE);
    gtk_combo_box_set_active(GTK_COMBO_BOX(bot_OBJECT("cbb_startMinute")), mS);
    gtk_combo_box_set_active(GTK_COMBO_BOX(bot_OBJECT("cbb_endMinute")), mE);
    passSignal = FALSE;
    on_combo_box_time_activated(NULL, NULL);
}
...
static gboolean btn_accept_clicked()
{
    ...
    {
        itemTmp = g_new0(botDayScheduleItem, 1);
        itemTmp->level = level * DIV_BRIGHTNESS;
        itemTmp->timeBegin = startTime;
        itemTmp->timeOn = endTime - startTime;
        dayTmp->itemlistTmp = g_slist_append(dayTmp->itemlistTmp, itemTmp);
    }
    bot_item_dialog_fill_tree(NULL);
    return TRUE;
...
static void on_btn_item_clicked(GtkButton *btn, gpointer data)
{
    ...
    case BTN_ITEM_EDIT:
        ...
        if(!itemTmp)
            return;
        sensitive = TRUE;
        bot_schedule_item_modify(itemTmp);
        break;
    case BTN_ITEM_REMOVE:
        itemTmp = NULL;
        if(gtk_tree_selection_get_selected(selection, &model, &iter))
    ...
    gtk_widget_set_sensitive(GTK_WIDGET(bot_OBJECT("frame_config_schedule")), sensitive);
    gtk_widget_set_sensitive(GTK_WIDGET(bot_OBJECT("dialog_action_area_schedule")), !sensitive);
}
...
static gboolean bot_schedule_item_element_init(void)
{
    ...
    /**signal connect*/
    g_signal_connect(bot_OBJECT("cbb_startHour"), "changed", G_CALLBACK(on_combo_box_time_activated), GINT_TO_POINTER(24));
    g_signal_connect(bot_OBJECT("cbb_endHour"), "changed", G_CALLBACK(on_combo_box_time_activated), GINT_TO_POINTER(24));
    g_signal_connect(bot_OBJECT("cbb_startMinute"), "changed", G_CALLBACK(on_combo_box_time_activated), GINT_TO_POINTER(59));
    g_signal_connect(bot_OBJECT("cbb_endMinute"), "changed", G_CALLBACK(on_combo_box_time_activated), GINT_TO_POINTER(59));

    g_signal_connect(bot_OBJECT("btn_item_add"), "clicked", G_CALLBACK(on_btn_item_clicked), GINT_TO_POINTER(BTN_ITEM_ADD));
    g_signal_connect(bot_OBJECT("btn_item_remove"), "clicked", G_CALLBACK(on_btn_item_clicked),GINT_TO_POINTER(BTN_ITEM_REMOVE) );
    g_signal_connect(bot_OBJECT("btn_item_edit"), "clicked", G_CALLBACK(on_btn_item_clicked), GINT_TO_POINTER(BTN_ITEM_EDIT));
    g_signal_connect(bot_OBJECT("btn_item_accept"), "clicked", G_CALLBACK(on_btn_item_clicked), GINT_TO_POINTER(BTN_ITEM_ACCEPT));
...
static void bot_program_popup_menu_clicked(GtkWidget *popup_menu_item, gpointer userdata)
{
    ...
    case bot_PROGRAM_M_REMOVE:
    {
        ...
        {
            botSoap bot_soap;
            gint soap_err = bot_remove_program_execute(&bot_soap,programID);
            if(soap_err == ns1__MESSAGE_USCORERESULT_USCOREEnum__NON_USCOREERR)
...
static gboolean bot_program_popup_menu_add_clicked(void)
{
    ...
    bot_DIALOG_SHOW(main_widgets->create_schedule_dialog);
    ...
    case DIALOG_OK_RESPONSE:
        ...
        else
        {
            ...
            if(gtk_tree_selection_get_selected(selection, &model, &iter))
            {
                ...
                gint soap_err = bot_add_program_execute(&bot_soap,zoneID, program_name, &program_id);
                if(soap_err == ns1__MESSAGE_USCORERESULT_USCOREEnum__NON_USCOREERR)
                {
    ...
    bot_DIALOG_HIDE(main_widgets->create_schedule_dialog);
    return exit_here;
}
